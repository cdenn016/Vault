...
**Participatory realism** is the interpretive program of the [[Gauge-Theoretic Multi-Agent VFE Model]] in which geometric, "physics-like" structure is not presupposed but *induced* from the informational dynamics of interacting epistemic agents. The name borrows Wheeler's slogan **"it from bit"** — that physical reality ("it") derives its existence from registered information ("bit") — and gives it a precise mathematical realization: a tensor on a noumenal base manifold $\mathcal{C}$ is constructed from a fiber-respecting information metric and an agent's belief/model section. The verified covariant version uses the section's connection-relative vertical first jet; the older horizontal frame-twist extension is a separate postulate.
...
> [!important] Covariant-pullback correction (2026-08-01)
> The verified construction in [[gauge-vfe-rg-pullback-geometry-2026-08-01]] sharpens the older
> "gauge-frame-dependent metric" wording. The rigorous vertical informational tensor is invariant
> under passive gauge changes but depends on the agent's section and on the selected connection.
...
- **It from bit** $\Leftrightarrow$ the covariant-vertical pullback of the [[Fisher information metric]] from the statistical fiber to the base, giving an agent- and connection-relative *emergent* semigeometry from belief fields.
...
- **Observer-dependent reality** $\Leftrightarrow$ dependence on the agent's statistical section and selected connection: different agents can induce different tensors on the *same* noumenal substrate. Passive gauge representatives describe the same tensor; the separate frame-twist extension has its own gauge-covariance problem.

The framework adopts a **pan-agentic, panprotopsychist** ontology: every system at every scale carries gauge-frame structure plus participation in variational dynamics on a statistical manifold appropriate to its scale ($\mathrm{GL}(K,\mathbb{R})$ on a real Gaussian fiber classically; $U(\mathcal{H})$ at the open scale-0 quantum level). An electron is a scale-0 agent, a molecule a higher-scale meta-agent, a brain higher still. This is the move that lets the single-agent [[parr-2022-active-inference|active-inference]] / [[friston-2017-active-inference-process-theory|free-energy]] substrate be generalized into a *multi-agent, gauge-theoretic* theory of co-constituted physical and cognitive structure.
...
Equipping the associated bundle $\mathcal{E} = P\times_G \mathcal{B}$ with a fiber-respecting bundle metric (vertical block = Fisher–Rao, horizontal block = a frame-twist quadratic form built from the connection itself) and pulling back along $\sigma_i^{(q)}$ gives the **induced metric** on $\mathcal{C}$:
...
The same construction applied to $p_i, s_i, r_i$ yields four coexistent tensors $G^{(q)}_i, G^{(p)}_i, G^{(s)}_i, G^{(r)}_i$, organized by timescale into **three tiers**: the belief tier $G^{(q)}_i$ is *epistemic* geometry (an instantaneous attention-and-uncertainty map); $G^{(p)}_i$ is *expectational* geometry; and the slow-channel $G^{(s)}_i, G^{(r)}_i$ are *structural* geometry. The author proposes that what an agent phenomenally experiences as "ambient space" is identified with the **structural tier $G^{(s)}_i$**, the slow generative-model pullback — mapping onto Kant's "form of intuition" rather than an instantaneous prior.

### Connection-relative covariant pullback and timeless histories

For a section $s$ and selected connection $\omega$, the exact vertical derivative is
$D^\omega s=\operatorname{ver}^\omega\circ Ts$. The rigorous informational pullback is

$$
h_s^\omega(X,Y)=g^F(D^\omega sX,D^\omega sY).
$$

It is globally defined and passive-gauge invariant under the stated equivariance hypotheses, but it
is connection-relative. If $\omega'=\omega+a$, then
$D^{\omega'}sX=D^\omega sX+R_a(X)$ and the metric changes by the corresponding two cross terms plus
$g^F(R_aX,R_aY)$. The base therefore acquires an **agent-perceived semigeometry** only relative to a
section and comparison rule; no canonical connection is supplied by the principal bundle alone.

At fixed $c\in\mathcal C$, a changing law is a curve inside one statistical fiber and is intrinsically
vertical. A whole-agent update is a curve of sections; its evaluation at each fixed context is again
vertical. The base remains fixed and timeless. VFE descent can orient an unparameterized inference
...
- [[fuchs2014-qbism-locality|fuchs-2014-qbism]], [[rovelli-1996-relational-qm]], [[seth-2021-being-you]], [[tononi-2016-iit]], [[VanRaamsdonk-2010-spacetime-entanglement|vanraamsdonk-2010-entanglement-spacetime]], [[zurek-2003-einselection|zurek-2003-decoherence]], [[page-wootters-1983]] — participatory / observer-relative / emergent-geometry neighbors discussed in the related-work and observer sections.
