...
First, **optimization**. Steepest descent in raw parameter coordinates depends arbitrarily on how the model happens to be parameterized. [[amari-1998-natural-gradient]] showed that the true steepest-descent direction on a statistical manifold is the ordinary gradient *preconditioned by the inverse Fisher metric*, $F^{-1}\nabla\mathcal{L}$ — the [[Natural gradient]]. The VFE transformer's M-step (parameter update) and its online belief updates are natural-gradient steps, so the Fisher metric is the preconditioner that makes those updates reparameterization-invariant and statistically efficient. [[khan-rue-2023-bayesian-learning-rule]] turns this principle into the program's actual learning rule: natural-gradient descent on the *natural* parameters of an exponential-family posterior — with the update reading off the expectation-parameter gradient — unifies Adam, Newton, variational inference, and online learning under one Fisher-preconditioned Gaussian belief update, which is the closed form the model's belief steps realize.
...
**Exponential families and dual flatness.** For an exponential family written in natural parameters, the Fisher information equals the Hessian of the log-partition function, making the manifold *dually flat*: it carries two flat affine connections (the $e$-connection and $m$-connection) that are mutually dual with respect to the Fisher metric. The Gaussian beliefs $(\mu, \Sigma)$ that the model maintains per token are exponential-family distributions, so their belief space is precisely such a dually flat manifold; its full Fisher-Rao geometry — with the joint mean-covariance coupling, geodesic ODEs, and Rao distance worked out for exactly this $(\mu,\Sigma)$ manifold — is the subject of [[skovgaard-1984-riemannian-geometry-normal-model]], distinct from the covariance-only SPD cone. The family of [[amari-2000-methods-information-geometry]] $\alpha$-connections interpolates between these duals and is the geometric backbone of the alpha/Renyi machinery; [[zhang-2004-divergence-duality-convex]] constructs that whole $\alpha$-divergence family from a single convex potential, each member inducing one common Fisher-type metric together with a dual pair of $\alpha$-connections and recovering the canonical Bregman/KL divergence of the dually flat space at $\alpha = \pm 1$ — the reason the program's Renyi/alpha objective sits inside one dually flat geometry with KL as the special case.
...
- [[nielsen-2020-elementary-introduction-information-geometry]] — pedagogical information-geometry survey grounding the model's KL divergences and natural-gradient M-step.
