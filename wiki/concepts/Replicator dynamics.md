...
The weaker links are conceptual and parallel rather than mechanical. The replicator equation is a first-order population flow and therefore a useful comparator for the program's first-order side, but it is not a VFE reduction. Its selection dynamics, evolutionary stability, and mutant-invasion criterion differ from the reversible scalar consensus energy. The conditional second-order belief model also requires an independently specified kinetic metric; simply reading Fisher precision as mass does not promote replicator dynamics or establish oscillation as a unique signature. Replicator dynamics belongs to [[Statistical physics of social systems and collective behavior]] as a foundational selection-dynamics neighbor of the program's restricted averaging result, not as an overdamped recovery.
...
- [[Opinion dynamics]] — the program's restricted first-order averaging correspondence; replicator dynamics is an adjacent benchmark, not a recovered flow.
