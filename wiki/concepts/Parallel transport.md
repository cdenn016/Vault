...
- **Riemannian optimization.** Updating beliefs and gauge parameters by [[Natural gradient|natural-gradient]] or Riemannian SGD steps repeatedly needs to relocate momentum and previous gradients to the current iterate; transport is what makes those quantities comparable across steps.
...
**Lie-group and gauge picture.** When the structure group is a Lie group G (here the block general-linear group GL(k)), transport along a path is an element of G obtained by integrating a Lie-algebra-valued connection. The model parameterizes group elements in the algebra via "phi" and maps to the group with an exponential/[[Baker-Campbell-Hausdorff formula|Baker–Campbell–Hausdorff]] (BCH) retraction — the same algebra-first strategy [[finzi-2020-lieconv]] uses for general Lie groups and [[thomas-2018-tensor-field-networks]] uses for SO(3). In this picture, transport, the BCH retraction, and the [[Gauge transformation|gauge transformation]] are facets of one Lie-group action. The Erlangen-Program synthesis of [[bronstein-2021-geometric-deep-learning]] frames grids, groups, and gauges under exactly this transport-and-equivariance blueprint, and [[cohen-2016-gcnn]] is its discrete-group ancestor. The fiber-bundle / induced-representation (Mackey-theory) foundation of [[cohen-2019-general-theory-equivariant]] makes the bookkeeping rigorous, putting equivariant linear maps in one-to-one correspondence with convolutions by steerable kernels constrained per irrep and classifying G-CNNs by symmetry group, base space, and field type — the same frame-as-fibre and irrep-block decomposition the project's per-token GL(k) construction uses.

**Exact transport vs. vector transport.** Genuine parallel transport requires solving the ODE along a geodesic, which is expensive. [[absil-2008-optimization-matrix-manifolds]] introduces **vector transport**: any smooth map that approximates parallel transport to first order and is compatible with a chosen **retraction** (a cheap surrogate for the exponential map). The VFE transformer's `spd_affine` retraction for covariances and BCH retraction for GL(k) gauge parameters are precisely such retractions, and the transports paired with them are first-order surrogates rather than exact geodesic transport.
...
- **BCH retraction and "phi."** The Lie-algebra "phi" parameterization with a BCH retraction realises group-valued transport in the algebra, mirroring the log-coordinate approach of [[finzi-2020-lieconv]] and the irrep/Clebsch–Gordan bookkeeping of [[thomas-2018-tensor-field-networks]] (see [[Clebsch-Gordan coefficients]], [[Irreducible representation]]).
...
- **Riemannian M-step.** Vector transport ([[absil-2008-optimization-matrix-manifolds]]) lets the M-step's Riemannian SGD ([[bonnabel-2013-riemannian-sgd]]) and [[Natural gradient|natural-gradient]] updates relocate gradients and momentum to the current iterate without leaving the manifold.
- **Holonomy and cocycles.** The model's cocycle-relaxation and [[Holonomy|holonomy]] terms quantify the path-dependence of transport around token loops, i.e. the curvature of the learned connection.
