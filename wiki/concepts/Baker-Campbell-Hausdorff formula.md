...
**Relation to the matrix exp/log maps.** BCH is the algebra-level shortcut around the matrix exponential and logarithm that would otherwise mediate group composition: instead of forming $g_1 = \exp(\phi_1)$, $g_2 = \exp(\phi_2)$, multiplying, and taking $\log(g_1 g_2)$ — each step an eigendecomposition or scaling-and-squaring routine — a truncated BCH series produces the composite generator directly from brackets of $\phi_1$ and $\phi_2$. That round trip is itself far from trouble-free: the canonical survey [[moler-vanloan-2003-nineteen-dubious-ways]] catalogues how badly the matrix exponential can be conditioned and why the eigenvalue method breaks down precisely for non-normal or defective matrices — the regime the gauge frames $\exp(\phi)$ enter when $\phi$ is symmetric rather than skew. When the round trip is unavoidable, the scaling-and-squaring with Padé approximation of [[al-mohy-higham-2009-scaling-squaring-matrix-exp]] — the algorithm behind `torch.matrix_exp` and `scipy.expm`, with its backward-error and overscaling analysis — governs the fp32 fidelity of every transport $\Omega = \exp(\phi_i)\exp(-\phi_j)$ and BCH retraction, the overscaling inflating error most for that same non-compact symmetric-$\phi$ sector. The flat-vector-space alternative of pushing matrices through a logarithm, composing by ordinary addition, and exponentiating back is exactly the **Log-Euclidean** trade-off of [[arsigny-2006-log-euclidean]], which buys commutativity and speed at the price of discarding the bracket corrections; BCH instead keeps the corrections to a chosen order. The affine-invariant SPD geometry of [[pennec-2006-affine-invariant-tensor]] sits at the curved, bracket-retaining end of the same spectrum.
...
- [[moler-vanloan-2003-nineteen-dubious-ways]] — canonical survey of matrix-exponential methods, pinning the conditioning of expm and the eigenvalue-method breakdown for the non-normal/defective (symmetric-$\phi$) regime.
- [[al-mohy-higham-2009-scaling-squaring-matrix-exp]] — the scaling-and-squaring plus Padé algorithm behind `torch.matrix_exp`/`scipy.expm`, whose backward-error and overscaling analysis governs the fp32 fidelity of every gauge transport and BCH retraction.
