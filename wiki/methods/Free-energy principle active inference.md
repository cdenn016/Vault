...
The VFE transformer takes the free-energy principle as motivation for its inner inference semantics, not as one literal training objective shared by every update. The retained route performs one target-blind model-channel refinement and then one target-blind belief refinement with $q_i^{(0)}=p_i=s_i^{(1)}$; decode cross-entropy is separate, so this is not a Neal–Hinton incremental-EM step. **What it borrows:** per-token Gaussian states; precision-weighted prediction-error dynamics recast as [[Precision weighting]] inside attention; and the attention-as-precision interpretation of the Gaussian-KL logit. The config flag named `precision_weighted_attention` is a distinct, default-off, query-independent reliability bias rather than the source of this structural precision weighting. [[gl-k-attention-2026-07-09-review-revision]]

**Where it differs:** the belief covariance is updated within its SPD family using belief-side Fisher/AIRM geometry. At each recorded source SHA, the committed optimizer gate and stored configuration route the frame table through AdamW, but every provenance record marks a dirty worktree and no dirty diff survives; the exact executed optimizer cannot be reconstructed. The configured frame conditioner and heavy-ball fields are inactive and do not define a Fisher natural gradient. The belief-side discrepancy can use an order-[[Renyi divergence|Rényi divergence]] with KL as its order-one limit. This is distinct from the Li–Turner variational Rényi bound and from Amari's [[Alpha-divergence|alpha-divergences]] and alpha-connections, and it does not turn the separate decode cross-entropy into the same variational bound. The model also adds a GL(k) gauge structure over belief frames, so token beliefs are compared after transport rather than in one fixed basis. These are model-specific constructions, not consequences of the free-energy principle. [[gl-k-attention-2026-07-09-review-revision]]
...
- [[vanerven-2014-renyi-kl]] — order-Rényi divergence and its KL limit.
- [[li-turner-2016-renyi-vi]] — the distinct variational Rényi bound.
- [[amari-2000-methods-information-geometry]] — Amari alpha-divergences and alpha-connections, distinct from both Rényi constructions.
- [[amari-1998-natural-gradient]] — natural-gradient background; in this project the established use is belief-side, not the audited frame M-step.
