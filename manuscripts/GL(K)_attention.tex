...
\paragraph{Gauge redundancy in the transformer's own parameterization.}
The factors $W_Q, W_K$ and $W_V, W_O$ are not separately identified by the function a head computes. For any $A \in \mathrm{GL}(d_{\text{head}})$ the substitution
%
\begin{equation}
W_Q \mapsto W_Q A, \qquad W_K \mapsto W_K A^{-\top}
\label{eq:qk_gl_redundancy}
\end{equation}
%
leaves $M = W_QW_K^\top$ unchanged, since $W_QA(W_KA^{-\top})^\top = W_QAA^{-1}W_K^\top$. Independently, for any $B \in \mathrm{GL}(d_{\text{head}})$ the substitution $W_V \mapsto W_VB$, $W_O \mapsto B^{-1}W_O$ leaves the composite output-value map unchanged. Each head therefore carries an exact $\mathrm{GL}(d_{\text{head}}) \times \mathrm{GL}(d_{\text{head}})$ redundancy of its parameterization, whose invariant content is the pair of composites rather than the four factors. This is the reason the transformer-circuits analysis treats the query-key and output-value circuits, rather than the individual projections, as the objects carrying the computation \citep{elhage2021mathematical}.

Three consequences should be separated.

First, the group coincides. The action in \eqref{eq:qk_gl_redundancy} is a $\mathrm{GL}(d_{\text{head}})$ action, and Section~\ref{sec:multihead} restricts the gauge algebra of the present construction to $\bigoplus_a \mathfrak{gl}(d_{\text{head}})$. A $\mathrm{GL}$ symmetry on the per-head space is therefore not an assumption this framework imports from outside; it is already present, exactly, in the parameterization of every trained transformer.

Second, and this is the substantive difference, the transformer's redundancy is \emph{global} where the present construction's is \emph{local}. In \eqref{eq:qk_gl_redundancy} the matrix $A$ is a single matrix per head applied identically at every token position, so the action is a symmetry of the parameter-to-function map and carries no positional content. The gauge action of Section~\ref{sec:glk_invariance} instead assigns an independent $g_i$ to each token. Promoting a global symmetry to a position-dependent one is what requires a connection, and the transport $\Omega_{ij}$ is that connection. The relation between the two constructions is stated most accurately as follows: the framework gauges a redundancy the transformer already possesses. This is a statement about parameterizations and does not by itself derive either architecture from the other.

Third, the redundancy sharpens rather than weakens the scope note above. What the framework does not derive is the invariant object $M$, not the redundant factors, while a transformer's $W_Q$ and $W_K$ individually carry $d_{\text{head}}^2$ parameters of pure convention. A comparison that places the framework's transport beside $W_Q$ or $W_K$ separately compares a gauge-invariant object with a gauge-dependent one.

\paragraph{Weight decay as a gauge-fixing term.}
The redundancy is exact for the function but not for an $L^2$ penalty on the factors, so weight decay selects a representative from each orbit. At fixed $M$ with singular value decomposition $M = U\Lambda V^\top$, minimizing $\|W_Q\|_F^2 + \|W_K\|_F^2$ subject to $W_QW_K^\top = M$ attains $2\|M\|_*$ at the balanced factorization $W_Q = U\Lambda^{1/2}$, $W_K = V\Lambda^{1/2}$, because $\|W_Q\|_F^2 + \|W_K\|_F^2 \geq 2\|W_Q\|_F\|W_K\|_F \geq 2\|M\|_*$ with equality only when the two factors carry equal singular values. Weight decay therefore exerts pressure toward the balanced slice of the redundancy orbit, which is a gauge-fixing condition in the sense used throughout this paper, and it is the same mechanism by which the flat parameterization directions of the present construction are lifted in practice.

Three scope conditions apply. The statement characterizes the minimizer of the penalty at fixed function; under a full training objective the selected point trades the penalty against the likelihood and need not be exactly balanced. Decoupled weight decay as implemented in AdamW is not gradient descent on the $L^2$ penalty, so the argument supplies the direction of the selection pressure rather than its fixed point. And the conclusion concerns only the redundancy displayed above, not directions in parameter space that change the computed function.

\paragraph{Rotary position frames as a partial gauge fixing.}
The query-key redundancy is further reduced by rotary embeddings. With the rotation of Section~\ref{sec:rope} inserted between query and key, invariance of the logit under \eqref{eq:qk_gl_redundancy} requires $A$ to commute with $R(\theta)$ at every realized offset. Generically in the frequency ratios $\theta_n$, the realized rotations generate a dense subgroup of the torus $\mathrm{SO}(2)^{d_{\text{head}}/2}$, whose centralizer in $\mathrm{GL}(d_{\text{head}})$ is the block-diagonal group acting on each two-plane as $aI + bJ$, with $J$ the generator of that plane's rotation. The residual redundancy is then isomorphic to $(\mathbb{C}^\times)^{d_{\text{head}}/2}$, of real dimension $d_{\text{head}}$ rather than $d_{\text{head}}^2$. Rotary embeddings therefore fix most of the query-key gauge freedom, which is a further respect in which they act as a frame choice rather than as a logit bias. The genericity hypothesis on the frequency ratios is required for the density statement and is not verified here.
...
\subsection{Paths from Structural to Derived Correspondence}
\label{sec:s_to_d}

The status column of Table~\ref{tab:fep_nn_correspondence} records where the framework shares a computational form with an architectural element without predicting that element uniquely. A correspondence is promoted from S to D only by exhibiting the element as a consequence of the variational principle or of an explicit limit of it. This subsection sets out what each promotion would require. None of the paths below is claimed here as achieved, and several would change the content of the framework rather than only its presentation.

\paragraph{Promotions available within the present construction.}
Four rows can be addressed without enlarging the generative model.

The observation readout $p(o \mid \mu) = \mathrm{Cat}(\softmax(W\mu))$ is graded S because the decode is an external map on a gauge-fixed sector, while the observation term $-\mathbb{E}_q[\log p(o \mid k)]$ that it instantiates is already graded D. Replacing the linear vocabulary projection by a divergence to the prior bank of Section~\ref{sec:embeddings}, so that the logit for type $v$ is $-D(q_i \| p_v)$, removes the external $W$ and returns the readout to the divergence sector. The row then follows from the observation term rather than sitting beside it. This is the narrowest promotion available and requires no new construction.

Mean centering is graded S against the decomposition $\mathfrak{gl}(K) = \mathfrak{sl}(K) \oplus \mathbb{R}$ of Section~\ref{sec:off_diagonal_mixing}. The trace direction is a one-parameter redundancy of the frame parameterization, and removing it is a gauge-fixing condition, which is a derived operation rather than a shared form. Stating the $\mathrm{SL}(K)$ slice and the projection onto it would promote the row.

Layer normalization is graded S because the norm condition $\|\mu_j\|^2 \approx C$ is assumed as the hypothesis under which the projected-key bias cancels. Imposing it instead as a constraint on the variational problem, and minimizing $\mathcal{F}$ subject to $\|\mu\| = C$, makes the associated retraction the derived update rather than an analogy, and converts the norm condition from a hypothesis about when the reduction holds into a property of the construction. The cost is that the constrained problem is a different variational problem, whose stationarity conditions and multiplier would have to be derived rather than asserted.

The comparison between $\alpha D_{\KL}(q_i \| p_i)$ and $L^2$ weight decay is graded S because the framework's divergence is over beliefs while decay acts on parameters. Placing a hyper-prior over the slow-subsystem parameters and taking the corresponding maximum a posteriori estimate would supply a parameter-space penalty from within the construction. The promotion carries a testable consequence that distinguishes it from the current practice it would explain: posterior concentration is observation-count dependent, so the derived penalty is frequency-weighted rather than uniform across a table's rows.

A fifth row is available with a caveat. The multi-head restriction and the rectangular subspace embeddings are graded S because the block partition is chosen. Positing a symmetry of the generative model and decomposing the belief space into isotypic components makes the commutant block-diagonal by Schur's lemma, so the head partition follows rather than being imposed. The caveat is that the resulting head subspaces are then determined by the representation theory rather than learned, which is a stronger and different claim than the transformer's, and the promotion would establish a related architecture rather than the transformer's own.

\paragraph{Promotions requiring an extension of the generative model.}
The largest cluster of S rows shares one obstruction. As recorded in Section~\ref{sec:transformer_limit}, a transport-derived compatibility is always invertible, while a transformer's $M_h^a$ may be singular, and the framework does not derive the rectangular embeddings $U_Q^a, U_K^a, U_V^a$. Both would follow if the inter-agent coupling were mediated by a lossy channel rather than by comparison of complete beliefs. Writing the alignment energy as $D(\Pi_a q_i \| \Pi_a \Omega_{ij} q_j)$ for a rectangular channel map $\Pi_a$ makes the induced compatibility $\Pi_a^\top \Sigma^{-1} \Pi_a$ singular by construction, supplies the value map as the channel's readout, and realizes multiple heads as multiple independent channels. The map $\Pi_a$ would be a parameter of the generative model estimated in the slow subsystem, which is a legitimate object rather than an imposed morphism. The open requirement is the transformation law for $\Pi_a$ under the local action, without which the extended energy is not gauge-covariant; this is the same gap identified for arbitrary $M$ in Section~\ref{sec:transformer_limit} and is not resolved by the channel construction alone.

The distance-decaying priors of Section~\ref{sec:nonuniform_priors} are graded S individually even though the additive untempered entry of $\log \pi$ into the logits is derived. What is undetermined is the functional form of $\pi$. Positing that sources are generated by a stationary hazard process on the index set makes $\pi_k$ its marginal transition kernel, under which exponential decay is a geometric hazard, a window prior is a finite-support hazard, and a bucketed relative bias is a piecewise-constant hazard. The promotion is constraining rather than permissive, since the slope then denotes a hazard rate and per-head slopes denote a mixture over hazard rates, both of which are estimable independently of the attention weights and could disagree with them.

Rotary position frames would promote as the abelian, flat, trivial-holonomy case of a relative Regime-I connection indexed by offset, consistent with the cocycle statement of Section~\ref{sec:rope} and with the edge-relaxed parameterization of Lemma~\ref{thm:vanishing_holonomy}.

The residual connection of Section~\ref{sec:residual_connections} requires a theorem rather than a construction. The additive form is already the retraction in Euclidean mean coordinates; what is not established is that prior anchoring supplies a restoring force sufficient to prevent rank collapse at depth. A stability result for the anchored update would promote the row, and such a result is vacuous unless the hyper-prior location is distinct from the origin, since an anchor at the origin contributes no term to the numerator of the fused mean and acts only as shrinkage.

\paragraph{Rows obstructed by autoregressive causality.}
Two rows should be marked as blocked rather than open. Identifying $\mathcal{F}$ with the training loss is obstructed because the exact objective's next-token target is unavailable to belief revision, as recorded in Section~\ref{sec:filtering_free_energy}; conditioning the recognition factor at position $t$ on $x_{t+1}$ is a leak under an evaluation protocol that runs the same path. Only the self-coupling surrogate is available, so the row admits improvement but not promotion. Likewise, the identification of layered composition with backpropagation, and of tied inner iterations with fixed-point descent, is licensed only in smoothing mode under step-size and regularity conditions not established here, and smoothing is the mode that consumes the unavailable target. These two rows are constrained by the autoregressive setting rather than by the state of the derivation.
