...
which has the standard transformer attention-weighting form after the structural compatibility $M$ is adjoined, up to the key-norm bias term that cancels under the layer-normalization or high-dimensional-concentration conditions established above. Here $\tau = \sqrt{d_k}$ is the $\kappa = 1$ case of the learnable attention temperature; the unreduced model retains the per-head sharpness scalar $\kappa_a$ of Eq.~\eqref{eq:per_head_temperature}, with $\kappa_a = 1$ matching the standard normalization.
...
When the key norms are exactly constant, the second logit term cancels under the row softmax and the attention weights have the exact identity-bilinear dot-product form. Layer normalization or high-dimensional concentration supplies only an approximate version of this norm condition.
...
Having established that attention emerges from the variational free energy, we now compare gradient descent on this free energy, in the deterministic identity-transport limit, with analogous standard-transformer training structures; general learned projection maps remain structural additions.
...
In the shared-frame specialization, Fisher preconditioning with $\Sigma_i = \sigma^2 I$ cancels the scalar $\sigma^{-2}$ factors in the boxed update (Eq.~\ref{eq:residual_gradient_flow} above), and $\Omega_{ij} = I$ leaves the alignment direction $\sum_j\beta_{ij}(\mu_i - \mu_j) = \mu_i - \sum_j\beta_{ij}\mu_j$. The neighbor term is therefore the exact identity-transport aggregation $\hat\mu_i=\sum_j\beta_{ij}\mu_j$. Applying a learned $W_V$ afterward defines the distinct structural output $z_i=W_V^\top\hat\mu_i=\sum_j\beta_{ij}V_j$; this value map and the general query-key compatibility $M$ are structural transformer maps, not absorbed nonidentity Regime-I transports. This is consistent with the E-step algorithm we implement, which applies Fisher preconditioning $\tilde{\nabla}_\mu \leftarrow \Sigma_i \cdot \nabla_\mu \mathcal{F}$ without imposing those structural projection maps.

\paragraph{General (non-isotropic) mean gradient.}
The preceding expressions assumed isotropic covariances $\Sigma_i = \sigma^2 I$ and identity transport $\Omega_{ij}=I$. We now state the fully general gradient, valid for arbitrary covariances $\Sigma_i$, edge-dependent gauge connections $\Omega_{ij}$, general prior precision $\Sigma_{p,i}^{-1}$, and state-dependent prior coupling $\alpha_i$ (Section~\ref{sec:state_dependent_precision}).
...
Under isotropic beliefs and the exact shared-frame Regime-I specialization $\Omega_{ij} = I$, variational natural-gradient descent on the free energy (which is $\mathrm{GL}(K)$-equivariant in the unrestricted framework by Theorem~\ref{thm:glk_invariance}) has the update
