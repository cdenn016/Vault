...
We report language-model experiments with a realized non-isotropic token-frame Regime-I model under diagonal-covariance and single-exponential-frame approximations. The model uses information divergences, vertex-frame transport, and Fisher natural-gradient belief dynamics without learned attention projections, MLPs, or pointwise activation functions; the headline runs retain one linear vocabulary projection, and the multi-head sweep additionally enables an identity-initialized head mixer absorbable into that linear decode (Table~\ref{tab:glk_spec}). On Japanese Wikipedia, a $\mathrm{GL}^+(15)$ configuration with embedding dimension $K = 90$ reaches test perplexity 24.2 in one seed, using R\'enyi-$\tfrac{1}{2}$ rather than the exactly derived KL for alignment. On WikiText-103, a one-seed $\mathrm{GL}^+(15)$ configuration reports test perplexity 71.6, compared with 118.6 for a standard transformer at matched embedding dimension and 48.5 for a parameter-matched standard transformer; a development-provenance $\mathrm{GL}^+(10)$ width sweep improves from 219.0 to $74.1 \pm 0.3$ over three seeds at its largest width. Frame diagnostics show categorical separation in a low-dimensional subspace and within-category diversification in the bulk. A frozen-BERT consistency check across 105 English passages finds agreement between KL-based and dot-product attention in the degenerate limit (grand mean $r = 0.804$, 95\% CI $[0.771, 0.838]$), much of which follows from the identity-transport algebra when projected-key norms are approximately constant. These experiments do not establish exact full-$\mathrm{GL}(K)$ equivariance of the realized diagonal-covariance model.
\end{abstract}
...
We then show that the shared-frame, isotropic Regime-I limit has identity transport and reduces exactly to an identity-bilinear dot-product score plus a key-norm bias. General transformer attention is obtained only after adding the structurally compatible bilinear $M = W_QW_K^\top$. We report a WikiText-103 language-model experiment with the realized non-isotropic token-frame Regime-I construction under diagonal-covariance and single-exponential-frame approximations, without learned attention projections, MLPs, or pointwise activation functions and with a single linear vocabulary decode. A frozen-transformer analysis separately checks the identity-transport score under approximately constant projected-key norms (Supplementary Appendix~E).
...
\paragraph{Envelope versus surrogate gradient conventions.} Two distinct gradient conventions arise depending on whether the attention distribution is treated as a function of the belief or held fixed at its optimum. The default canonical configuration, in which the attention-entropy term of \eqref{eq:F_align_canonical_tau} is retained so that $\beta_{ij}^*$ is the genuine row-Lagrangian stationary point, detaches $\beta_{ij}^*$ from the belief and descends the envelope form \eqref{eq:envelope_form}: its implemented gradient is $\sum_j \beta_{ij}^* \partial E_{ij}/\partial x$ with no $\partial\beta_{ij}^*/\partial x$ contribution, because the cross-term $\sum_j(\partial\mathcal{F}/\partial\beta_{ij})(\partial\beta_{ij}^*/\partial x)$ vanishes at $\partial\mathcal{F}/\partial\beta_{ij} = 0$. This is the gradient of the reduced free energy $\mathcal{F}_{\mathrm{red}}$, and it is the form the reference implementation evaluates by default. The alternative, entropy-suppressed surrogate path drops the attention-entropy term and instead differentiates the composition $\sum_j \beta_{ij}^*(x) E_{ij}(x)$ through the softmax via automatic differentiation, which applies the product rule and produces additional $(\partial\beta_{ij}^*/\partial x) E_{ij}$ terms. To compare like with like, let $P(x)$ collect the identical prior, self-coupling, and observation terms in the two objectives and define
\begin{equation}
J_{\mathrm{surr}}(x) = P(x) + \langle E\rangle_{\beta^*},
\qquad
J_{\mathrm{can}}(x) = P(x) - \tau\log Z_i.
\label{eq:canonical_surrogate_objectives}
\end{equation}
The second convention computes $\langle E \rangle_{\beta^*} = \sum_j \beta_{ij}^* E_{ij}$ in place of the canonical row term. At $\beta^*$,
\begin{equation}
-\tau\log Z_i
=\langle E\rangle_{\beta^*}
+\tau\sum_j\beta_{ij}^*\log(\beta_{ij}^*/\pi_j),
\label{eq:canonical_row_decomposition}
\end{equation}
so $J_{\mathrm{can}}$ contains the positive attention-entropy regularizer omitted from $J_{\mathrm{surr}}$. Using the softmax sensitivity $\partial \beta_{ij}^*/\partial x = -\tau^{-1}\beta_{ij}^*\big(\partial E_{ij}/\partial x - \langle \partial E/\partial x\rangle_{\beta^*}\big)$, the difference between the two gradients takes the covariance form
\begin{equation}
\nabla_x J_{\mathrm{surr}} - \nabla_x J_{\mathrm{can}}
= -\tau^{-1} \mathrm{Cov}_{\beta^*} \big(E_{ij}, \partial E_{ij}/\partial x\big).
\label{eq:autograd_envelope_gap}
\end{equation}
The two vector fields coincide if and only if the covariance in Eq.~\eqref{eq:autograd_envelope_gap} vanishes. Equal energies on the active keys, or equal energy derivatives on those keys, are sufficient conditions. For a fixed finite row whose energies and derivatives remain bounded, the covariance is bounded and the prefactor $\tau^{-1}$ makes the gap vanish as $\tau \to \infty$. Canonical stationarity alone is not sufficient. For example, take $\tau=1$, a uniform two-key prior, $E_1(x)=x$, $E_2(x)=0$, and $P(x)=\tfrac{1}{2}(x-a)^2$ with $a=(2+e)/(1+e)$. At $x=1$, $\beta_1^*=1/(1+e)$ and $J_{\mathrm{can}}'(1)=0$, while $J_{\mathrm{surr}}'(1)=-e/(1+e)^2\neq0$. The $\partial\beta/\partial x$ terms, which vanish in the reduced-free-energy gradient by the envelope theorem, are present only in the surrogate (through-the-softmax) gradient, where they provide a softmax-gradient nonlinearity (Section~\ref{sec:ffn_nonlinearity}). Standard transformer training follows this surrogate convention. The general gradient expressions and natural-gradient flows stated below (Eqs.~\ref{eq:mu_gradient_general} and \ref{eq:natural_gradient_general}, and the residual flow of Eq.~\ref{eq:residual_gradient_flow}) display the $\partial\beta_{ij}/\partial x$ terms explicitly because they correspond to this surrogate path, which is also the form used in the supplementary covariance derivation; under the default envelope convention adopted here those terms drop, leaving the $\sum_j \beta_{ij}^* \partial E_{ij}/\partial x$ summand of \eqref{eq:envelope_form}.
...
There is a single objective here, the reduced free energy $\mathcal{F}_{\mathrm{red}}$ of \eqref{eq:free_energy_final}. Under the causal attention prior its coupling sum runs only over $j \leq i$, since $\beta_{ij}^* = 0$ for $j > i$, so the support of the pairwise alignment terms is already the past of each position. What distinguishes the default scheme from the alternative is not a second objective but the update rule applied to this one. The default reference implementation performs one row-local natural-gradient filtering step: each belief $q_i$ is updated using only the query-side partial of its own position-$i$ contribution to $\mathcal{F}_{\mathrm{red}}$, holding the remaining beliefs fixed and treating the appearance of $q_i$ as a transported key in later positions' energies as detached. This finite step is not an exact row argmin or coordinate-ascent update, and it supplies no EM monotonicity guarantee for the full $\mathcal{F}_{\mathrm{red}}$. The query-side contribution at position $i$ collects the prior terms, the row sum $\sum_{j \leq i} \beta_{ij}^* \partial E_{ij}/\partial x$ over the position's own past, and the observation terms, for a belief parameter $x \in \{\mu_i, \Sigma_i, \phi_i\}$. The mean component of the alignment partial is the query-side derivative
\begin{equation}
...
\end{equation}
This differs from the gradient of the reduced free energy $\mathcal{F}_{\mathrm{red}}$ (Eq.~\ref{eq:free_energy_final}) by the covariance term $-\tau^{-1}\mathrm{Cov}_{\beta^*}(E_{ij}, \partial E_{ij}/\partial q_i)$ of Eq.~\ref{eq:autograd_envelope_gap}; equality of the vector fields requires that term to vanish and does not follow from canonical stationarity. Here $o_i$ denotes the present-token evidence available to the recognition factor at position $i$. In the autoregressive language-modeling implementation the next-token prediction loss is evaluated only at the M-step (Algorithm~\ref{alg:em_loop}); the E-step belief inference is target-blind, so no future token enters the fast dynamics.
...
Layer normalization \citep{ba2016layernorm} fixes the norm of a pre-projection activation only up to its learned affine parameters. It can make projected-key norm constancy plausible when the activation statistics and key projection do not reintroduce substantial token-dependent norm variation, but it does not enforce that condition; Appendix~E measures the residual spread. Under this conditional approximation, the key bias is
...
where $\mathfrak{sl}(K)$ consists of traceless matrices (volume-preserving transformations) and $\mathbb{R}$ is the trace component (overall scaling). The exact gauge-side bias $\|\mu_j\|^2$ depends on the belief-mean magnitude; in the structural transformer comparison, the corresponding softmax-removable term depends on the projected-key norm. If layer normalization and the subsequent key projection yield approximately constant projected-key norms, the resulting norm control plays a role analogous to suppressing the scaling degree of freedom. The condition is only approximate in the measured setting, where Appendix~E reports $\mathrm{CV}(\|K\|^2) \approx 0.24$. In the strict identity-transport reduction, exact belief-mean norm constancy leaves the belief-content dot product as the nonconstant part of the logit.
...
where $O(1)$ represents the approximately constant key bias under the projected-key norm condition.
...
In high-dimensional spaces, dot products $Q_iK_j^\top$ have standard deviation $O(\sqrt{d_k})$ for independent unit-variance components. If the residual projected-key bias fluctuations remain $O(1)$ under the stated normalization and projection conditions, normalizing pre-softmax logits to $O(1)$ variance requires
...
When the key norms are exactly constant, the second logit term cancels under the row softmax and the attention weights have the exact identity-bilinear dot-product form. Layer normalization or high-dimensional concentration may supply an approximate version only under the stated activation and projection conditions.
...
The natural-gradient label here is specific to the belief channel: the $(\mu_i, \Sigma_i)$ updates use the Fisher-Rao metric of the Gaussian manifold, exact for these coordinates. The gauge frame $\phi_i$ instead uses the implemented regularized Cartan/Killing Lie-algebraic preconditioner
\begin{equation}
...
where $\tilde{\nabla}_{\theta_i}$ denotes a geometry-aware preconditioned gradient: the Fisher-Rao natural gradient for $\mu_i$ and $\Sigma_i$, and the $\mathrm{O}(K)$-invariant regularized Cartan/Killing preconditioner of Eq.~\eqref{eq:regularized_cartan_killing_preconditioner} for $\phi_i$. The latter is not a full-$\mathrm{GL}(K)$-covariant natural gradient. Each channel possesses its own residual structure and effective learning rate. This multi-channel residual flow corresponds to the residual stream interpretation of transformers \citep{elhage2021mathematical}, where each layer reads from and writes to a shared residual pathway. In the gauge-theoretic framework, this residual stream is the belief state $(\mu_i, \Sigma_i, \phi_i)$, accumulating incremental updates from successive layers of variational refinement.
...
An important caveat: the high correlation follows in large part from an algebraic identity. When projected-key norms are approximately constant, $\|Q_i - K_j\|^2$ and $-2Q_i K_j^\top$ differ only by terms approximately constant under softmax; layer normalization alone does not enforce this condition after the key projection. The non-trivial content lies in the quantitative predictions: the temperature scaling $\tau \approx 2\sqrt{d_k}$, the key-norm bias (Cohen's $d = 1.43$, 94\% HDI $[1.08, 1.88]$; this effect-size posterior is preliminary, with limited effective sample size and $\hat{R} \approx 1.02$, marginally above the convergence guideline), the per-head temperature structure, and their cross-model variation. We therefore treat the BERT analysis as a \emph{consistency check} of the degenerate limit, while the GL($K$) language-model experiment (next section) probes the realized framework.
...
\end{equation}
The $n^2$ inter-block edges therefore contain only the $2n$ vertex perturbations, with the common mode canceling. If the $X_i$ are centered, independent, and finite variance, then $\bar X_A-\bar X_B=O_{\mathrm{RMS}}(n^{-1/2})$, so
\begin{equation}
\bigl(g_1^{(\mathrm{orig})}\bigr)'=O_{\mathrm{RMS}}(n^{-1/2}),
\qquad
g_2'=O_{\mathrm{RMS}}(n^{-1/2}),
\qquad
y_1=y_2=-\tfrac{1}{2}.
\label{eq:rg_scaling}
\end{equation}
...
The $n^{-1}$ rate belongs instead to an explicitly enlarged independent-edge ensemble. If the $n^2$ centered, finite-variance variables $\delta\Omega_{ij}$ between two blocks are mutually independent, their arithmetic average is $O_{\mathrm{RMS}}(n^{-1})$. A linearized holonomy built from independent coarse-grained edge variables has the same rate, giving $y_2=y_3=-1$ and $y_3^{(\mathrm{action})}=-2$ for the squared norm. This ensemble is not the realized vertex-frame Regime I and is not established by the data-dependent Regime-II parameterization, whose edges share vertex variables and learned weights.

\begin{proposition}[Conditional contraction in vertex and edge ensembles]
\label{thm:rg_conditional}
Let $\mathcal{T}(K, N, g_1^{(\mathrm{orig})}, g_2, g_3)$ denote the family of gauge VFE models. Assume centered, independent, finite-variance vertex perturbations $\Delta_i$ and $X_i$ in Regime I. For the separate edge-ensemble clause, assume centered, independent, finite-variance edge perturbations $\delta\Omega_{ij}$. Then:
\begin{enumerate}
...
\item \emph{Coarse-graining exponents on trained models.} Extract attention matrices from trained checkpoints, apply spectral coarse-graining, and measure how $g_1$ and $g_2$ scale with cluster size. Measure $g_3$ only for a non-flat edge model, because vertex-frame Regime I has $g_3=0$ identically. These tests would determine which, if either, of the vertex or independent-edge premises approximates learned representations.
...
The $\mathrm{GL}(K)$ invariance theorem (Theorem~\ref{thm:glk_invariance}) governs unrestricted full Gaussian covariances under common invertible pushforwards. The realized model is narrower: it uses diagonal covariances and single-exponential $\mathrm{GL}^+(K)$ vertex frames, with diagonal projection under general congruence understood as an approximation. In its strict shared-frame reduction, $\Omega_{ij}=I$ and the exact compatibility is $\sigma^{-2}I$; a general $W_QW_K^\top$ is not Regime-I transport without an additional transformation law. The one-seed $\mathrm{GL}^+(15)$ WikiText-103 run reports test perplexity 71.6 without MLPs, learned attention projections, or pointwise activation functions, compared with 118.6 for a standard transformer at matched embedding dimension and 48.5 for a parameter-matched standard transformer. A development-provenance $\mathrm{GL}^+(10)$ width sweep reports monotone improvement from PPL $219.0 \pm 1.3$ at $K = 10$ to $74.1 \pm 0.3$ at $K = 120$ over three seeds per width (Appendix~\ref{app:vfe3_scaling}). Frame diagnostics show categorical separation in a low-dimensional subspace and within-category diversification in the bulk. The frozen-BERT consistency check finds quantitative agreement between identity-transport KL scores and dot-product attention under approximately constant projected-key norms; it does not independently support the unrestricted gauge theorem (Supplementary Appendix~E).
