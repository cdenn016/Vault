...
where each expression is the same logit kernel $\sigma^{-2}\Omega_{ij}^{-\top}$ appearing in $\tilde{s}_{ij} = \mu_i^\top(\cdot)\mu_j$, restricted to a progressively larger class of admissible transports $\Omega_{ij}$. Standard attention uses a constant, learned gauge with $W_Q W_K^\top = \sigma^{-2}\Omega^{-\top}$ (Limit~2 fully imposed). RoPE augments this with a fixed, position-dependent $\mathrm{SO}(2)^{d_k/2}$ rotation sandwiched between the projections (Limit~2 partially relaxed for positional information). The full gauge-theoretic framework allows the complete transport $\Omega_{ij}$ to be agent-pair-dependent and learned (Limit~2 fully relaxed). This hierarchy clarifies the design space such that positional encoding methods can be understood as choices about which subgroup of $\mathrm{GL}(K)$ carries position-dependent structure, and which components remain position-independent and learned from data.
...
We utilize an expectation-maximization structure as follows. This is a structural EM correspondence: the E-step and M-step descend distinct objectives (respectively, the belief-side alignment-plus-self-coupling energy and the cross-entropy learning loss) rather than coordinate-ascent steps on a single shared free energy, so no monotone-evidence guarantee is claimed; the Supplementary decode derivation (Section~\ref{sec:supp_decode_derivation}) makes the same point for the learning loss.
