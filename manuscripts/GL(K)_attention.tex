...
precisions, while agents near the prior retain strong prior anchoring. This then is the covariance analog of the adaptive weight decay described in
...
Optimizer & Adam ($\mu, \Sigma, W_{\text{out}}$); heavy-ball momentum 0.9 ($\phi$) \\
Learning-rate schedule (warmup, decay) & 100-step warmup; decay to 1\% of peak \\
Batch size (sequences; tokens per step) & 64; 8{,}192 \\
Token budget (total training tokens; steps) & 491.5M; 60{,}000 \\
Gradient clipping (max norm) & none \\
Weight decay / prior-coupling $\alpha$ & 0.02 ($\mu, \Sigma, W_{\text{out}}$), 0.05 ($\phi$); $\lambda_\alpha = 1$ (state-dependent) \\
\hline
\multicolumn{2}{l}{\textit{Variational inference}} \\
Attention temperature $\kappa$ ($\tau = \kappa\sqrt{d_{\text{head}}}$) & 1.0 \\
Belief-mean learning rate $\eta_q$ ($\mu$) & 0.9 (E-step); 0.015 (M-step) \\
Covariance learning rate $\eta_\Sigma$ ($\Sigma$) & 0.001 (E-step); 0.0045 (M-step) \\
Gauge-frame learning rate $\eta_\phi$ (E-step; M-step) & 0 (frozen); 0.015 \\
Prior precision $\alpha$ (or state-dependent $b_0, c_0$) & state-dependent, $b_0 = c_0 = 1.0$ \\
Inner VFE iterations $T$ per layer & 1 \\
\hline
\multicolumn{2}{l}{\textit{Protocol}} \\
Seeds (count; values) & 3; \{6, 23, 64\} \\
Checkpoint policy (selection metric, frequency, early stopping) & best validation PPL; eval every 1{,}500 steps; no early stopping \\
Hardware (GPU; memory; wall-clock) & RTX 5090; 64GB; 2.0--7.7 h per run (K = 10 to 120) \\
\hline
\end{tabular}
\caption{Training hyperparameters for the gauge VFE language-model runs. The architectural settings that differ across runs (gauge group, $K$, heads, divergence, positional encoding, E-step gauge-frame schedule) are recorded in Table~\ref{tab:glk_spec}; this table collects the optimization and protocol settings of the reproducible $\mathrm{GL}^+(10)$ embedding-dimension sweep (\texttt{train\_vfe.py}), read from its archived configuration dictionaries. The headline Japanese-Wikipedia and English-comparison runs were produced by an earlier implementation and differ as noted in Table~\ref{tab:glk_spec}; their archived dictionaries are distributed with the code. The M-step updates $\mu, \Sigma, W_{\text{out}}$ by Adam and the gauge frame $\phi$ by heavy-ball momentum (not a natural-gradient preconditioner); the E-step freezes $\phi$ and updates $\mu, \Sigma$ by Fisher natural gradient.}
