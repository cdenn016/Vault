...
The gauge-covariant attention $\beta_{ij} = \operatorname{softmax}(-D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j)/\tau)$ derived in the preceding section is already a complete attention mechanism. Standard transformer attention can be compared with it along two distinct routes. The first route (Section~\ref{sec:untied_qk}) retains $\Omega_{ij} = U_iU_j^{-1}$ non-trivially throughout and exposes untied query-key projections $Q_i = U_i^{-1}\mu_i$, $K_j = U_j^\top\Sigma_j^{-1}\mu_j$ directly from the transported KL, with the implicit pair-dependent bilinear ranging over $\mathrm{GL}(d_k)$ as the per-token frames and precisions vary; this route is structurally minimal in the sense that no object beyond the existing belief and frame fields is introduced. The second route (Section~\ref{sec:dot_product_derivation}) takes isotropic covariances and shared frames, which force $\Omega_{ij} = I$, and then introduces a learned shared compatibility $M = W_QW_K^\top$ acting on the means. The identity-transport KL yields the exact scaled identity bilinear and its key-norm bias; arbitrary $M$ fills the cross-coupling slot only as a structural addition. The two routes reach comparable scaled dot-product forms only after their distinct closure conditions and shared key-norm control are imposed, and before those conditions they leave different residual key-side bias terms. The first route additionally captures rotary positional structure without introducing the separate $M$. Our experiments (Section~\ref{sec:glk_lm}) implement the general gauge model without these reductions.
...
\end{equation}
%
and the actual transformer logit kernel is its low-rank lift $W_Q^a (W_K^a)^\top = U_Q^a M_h^a (U_K^a)^\top$. The SVD identity is exact, but interpreting an arbitrary $M_h^a$ as a gauge transport is not: it is a structural compatibility morphism on the head subspace. The strict Regime-I reduction occupies only the special point $M_h^a = \sigma^{-2}I$ after the head subspace is fixed.

\paragraph{Scope of the head-space compatibility.} The framework does \emph{not} derive either a general invertible per-head bilinear $M_h^a$ or the rectangular subspace embeddings $U_Q^a, U_K^a, U_V^a$, which select which $d_{\text{head}}$-dimensional subspace of the $d_{\text{model}}$-dimensional embedding space each head reads from. Both the arbitrary compatibility and the subspace selection are structural design choices. The gauge model independently admits a block-diagonal $\mathrm{GL}(d_{\text{head}})^H$ restriction, but that group structure does not turn the transformer's learned $M_h^a$ into Regime-I transport.
...
For $M = \sigma^{-2}I$, this is the exact Regime-I identity-transport cross term. For arbitrary $M$, the structurally extended logit takes the standard dot-product form
...
Identity transport gives $\hat\mu_i = \sum_j\beta_{ij}\mu_j$ directly. Applying a learned value map commutes with this sum, $W_V^\top\hat\mu_i = \sum_j\beta_{ij}W_V^\top\mu_j = \sum_j\beta_{ij}V_j$. The standard transformer's rectangular $W_V^a \in \mathbb{R}^{d_{\text{model}} \times d_{\text{head}}}$ and its thin-SVD factors are structural value/readout maps rather than absorbed Regime-I transport.
...
The comparison with standard scaled dot-product attention has three logically different ingredients. Shared frames in Regime~I and isotropic covariance are exact algebraic specializations, giving $\Omega_{ij} = I$ and the identity-bilinear logit of Eq.~\eqref{eq:structural_compatibility} at $M = \sigma^{-2}I$. Key-norm constancy, supplied either by layer normalization or by high-dimensional concentration of $\|\mu_j\|^2$, is approximate unless the norms are exactly equal. Replacing $\sigma^{-2}I$ by a general learned $M = W_QW_K^\top$ is a structural compatibility extension rather than a gauge specialization.
...
the canonical scaled dot-product attention of \citet{vaswani2017attention}. The strict Regime-I instance has $M_h^a = \sigma^{-2}I$; the ambient transformer kernel $W_Q^a (W_K^a)^\top$, of rank at most $d_{\text{head}}$, is the low-rank lift of a general structural head-space compatibility through the isometric subspace embeddings $U_Q^a, U_K^a$.
...
Each geometric reduction discards a specific degree of freedom and can be independently relaxed. The general transformer's learned compatibility is instead an added structural degree of freedom, so the comparison separates gauge reductions from architectural additions. Non-isotropic beliefs, learned pair-dependent gauge transport, and position-dependent connections remain genuine gauge-theoretic generalizations.
...
Having established that attention emerges from the variational free energy, we now show that, in the deterministic shared-frame limit with identity transport, gradient descent on this free energy recovers structures analogous to standard transformer training dynamics, with general learned projection maps treated structurally.
...
In the deterministic shared-frame limit ($\Omega_{ij} = I$), each belief becomes a narrow Gaussian $q_i = \mathcal{N}(\mu_i, \epsilon I)$ with $\epsilon \to 0^+$, and each prior is isotropic: $p_i = \mathcal{N}(\mu_{\text{prior}}, \sigma_p^2 I)$. The self-coupling term from the adaptive free energy \eqref{eq:free_energy_adaptive} decomposes as:
...
\begin{equation}
\mathcal{F}[\{\mu_i\}] = \underbrace{\sum_i \frac{\lambda_{p,i}}{2}\|\mu_i - \mu_{\text{prior}}\|^2}_{\text{Adaptive prior regularization}} + \underbrace{\sum_{i,j} \frac{\beta_{ij}}{2\sigma^2}\|\mu_i - \mu_j\|^2}_{\text{Alignment coupling}} - \underbrace{\sum_i \log p(o_i \mid \mu_i)}_{\text{Observation likelihood}}.
...
\begin{equation}
\mathcal{F}[\{\mu_i\}] = \underbrace{\sum_i \frac{\lambda_p}{2}\|\mu_i - \mu_{\text{prior}}\|^2}_{\text{L2 regularization (weight decay)}} + \underbrace{\sum_{i,j} \frac{\beta_{ij}}{2\sigma^2}\|\mu_i - \mu_j\|^2}_{\text{Alignment coupling}} - \underbrace{\sum_i \log p(o_i \mid \mu_i)}_{\text{Observation likelihood}}.
...
The first term, $\lambda_p(\mu_i - \mu_{\text{prior}})$, as discussed, matches weight decay or L2 regularization. The second term, $\sum_j \beta_{ij}(\mu_i - \mu_j)/\sigma^2$, is the gradient of the identity-transport alignment energy with respect to $\mu_i$: it pulls $\mu_i$ toward its neighbors, and under natural-gradient preconditioning the neighbor contribution targets the mixture-posterior aggregation $\hat\mu_i$ of Eq.~\ref{eq:glk_mixture_aggregation}. The third term, $\sum_j (\partial \beta_{ij}/\partial \mu_i)\|\mu_i - \mu_j\|^2/(2\sigma^2)$, represents gradient flow through the attention weights themselves, a non-linear ``activation'' that arises from differentiating the attention-weighted energy $\sum_j \beta_{ij} E_{ij}$ via the product rule (the autograd gradient). This term is absent from the gradient of the reduced free energy $\mathcal{F}_{\mathrm{red}}$ by the envelope theorem (Section~\ref{sec:final_free_energy}), but is present in standard autograd-based implementations that differentiate through the softmax. The fourth term, $-\frac{\partial \log p(o_i \mid \mu_i)}{\partial \mu_i}$, is the gradient of the loss function (i.e.\ the supervised learning signal).
...
In the shared-frame specialization, Fisher preconditioning with $\Sigma_i = \sigma^2 I$ cancels the scalar $\sigma^{-2}$ factors in the boxed update (Eq.~\ref{eq:residual_gradient_flow} above), and $\Omega_{ij} = I$ leaves the alignment direction $\sum_j\beta_{ij}(\mu_i - \mu_j) = \mu_i - \sum_j\beta_{ij}\mu_j$. The neighbor term is therefore the exact identity-transport aggregation. Applying a learned $W_V$ afterward gives $\sum_j\beta_{ij}V_j$ by linearity, but this value map and the general query-key compatibility $M$ are structural transformer maps, not absorbed nonidentity Regime-I transports. This is consistent with the E-step algorithm we implement, which applies Fisher preconditioning $\tilde{\nabla}_\mu \leftarrow \Sigma_i \cdot \nabla_\mu \mathcal{F}$ without imposing those structural projection maps.
...
\hline
\end{tabular}
\caption{Comprehensive correspondence between the gauge-theoretic VFE framework and transformer architecture. \textbf{Status column:} D = derived from the variational principle (mathematically exact or follows from explicit limits); S = \emph{structural} correspondence (the framework explains the component's role but does not uniquely predict its specific form, including the arbitrary compatibility $M = W_QW_K^\top$ and rectangular head-space maps); I = \emph{interpretive} correspondence (the framework and the architectural element belong to a common family but are not functionally identical; e.g., the Boltzmann gate of Eq.~\ref{eq:vfe_glu} uses the Gaussian PDF while GELU uses the Gaussian CDF).}
...
where $R(\theta_{j-i}) \in \mathrm{SO}(2)^{d_k/2}$ is sandwiched between the learned projections because it acts in the projected $d_k$-dimensional space. The rotation itself is a position-dependent Regime-I vertex-frame transport restricted to the compact Abelian subgroup $\mathrm{SO}(2)^{d_k/2} \subset \mathrm{GL}(d_k)$, whereas $W_Q$ and $W_K$ supply structural compatibility maps. Thus RoPE relaxes the shared-frame identity restriction on the positional transport without turning the arbitrary learned bilinear into transport. The bundle remains flat: $R(\theta_{j-i}) = \exp(-\phi_i^{(\mathrm{pos})})\exp(\phi_j^{(\mathrm{pos})})$ satisfies the cocycle condition and has trivial discrete holonomy.
...
\underbrace{\Omega_{ij}=R(\theta_i-\theta_j)\in\mathrm{SO}(2)^{d_k/2}}_{\text{RoPE positional transport}} \supset
\underbrace{\Omega_{ij}=I}_{\text{Shared-frame transport}},
\end{equation}

This hierarchy concerns only the admissible Regime-I transports. Standard attention without positional rotation occupies the shared-frame endpoint $\Omega_{ij} = I$ in this comparison, while its general learned $M = W_QW_K^\top$ remains a separate compatibility morphism. RoPE supplies a fixed position-dependent $\mathrm{SO}(2)^{d_k/2}$ transport between those structural maps. The full gauge model allows agent-pair-dependent vertex-frame transport in $\mathrm{GL}(K)$. Positional methods can therefore be compared by the subgroup carrying position-dependent transport without conflating that transport with the learned query-key bilinear.
...
Under isotropic beliefs and the exact shared-frame Regime-I specialization $\Omega_{ij} = I$, with the scalar $\sigma^{-2}$ absorbed at finite $\sigma$, variational natural-gradient descent on the free energy (which is $\mathrm{GL}(K)$-equivariant in the unrestricted framework by Theorem~\ref{thm:glk_invariance}) is:
...
The isotropic, shared-frame reduction of Section~\ref{sec:transformer_limit} can be viewed as a \emph{renormalization group (RG)-style coarse-graining flow} on the space of gauge-theoretic VFE models. The arbitrary transformer compatibility $M$ is not a coupling in this flow. Under an i.i.d.\ independence assumption on token-level perturbations, the central limit theorem delivers a conditional contraction result (Proposition~\ref{thm:rg_conditional} below). The Proposition is a direct application of the CLT to the coarse-graining definitions in Eqs.~\eqref{eq:g1_def}--\eqref{eq:g3_def}; its content lies in the i.i.d.\ premise, not in any gauge-theoretic structure. We separately conjecture, but do not claim to have validated on trained models, that the identity-transport comparison point is a stable infrared fixed point of this flow when the i.i.d.\ assumption is replaced by empirical attention-graph statistics (Conjecture~\ref{conj:rg_universality}).
