...
Three additional baselines at $d_{\text{model}} = 90$ with 9 heads isolate specific architectural contributions (Table~\ref{tab:glk_results}, ablation rows). \emph{Attention-only} (no MLP) achieves test PPL~142.8, confirming that removing the MLP from a standard transformer is not sufficient to match the gauge VFE's performance, so the advantage is not merely from operating without an MLP. Whether it stems specifically from the gauge-transport geometry, rather than from the diagonal-covariance beliefs or the KL-divergence attention, is settled by the controlled gauge-frame ablation reported below (\emph{Direct gauge-transport and attention-entropy ablations}). \emph{RoPE} at $d_{\text{model}} = 90$ achieves test PPL~138.6, indicating that positional encoding alone provides only a modest improvement at this embedding dimension. \emph{Parameter-equalized} (wider MLP, 9.2M params vs.\ 4.6M for the embed-matched baseline) achieves test PPL~145.8, showing that simply increasing MLP width does not recover the gauge VFE's performance.
...
\paragraph{Direct gauge-transport and attention-entropy ablations.}
The comparisons above vary the baseline architecture; two further ablations instead hold the gauge VFE architecture fixed and toggle a single internal mechanism, isolating the contributions of gauge transport and of the canonical attention-entropy term. These sweeps use a smaller, faster operating point of the V3 reference implementation (embedding dimension $K = 20$, single layer, single seed, $15{,}000$ updates, test perplexity near $144$), so their perplexities are not commensurable with the $K = 90$ results of Table~\ref{tab:glk_results}; they are reported as within-configuration relative contrasts. The three-seed embedding-dimension sweep of Supplementary Appendix~J fixes the across-seed noise floor at a coefficient of variation of $0.6$--$1.1\%$, below which a single-seed difference is not interpretable.

Varying only the gauge frame, a model with a learned $\mathrm{GL}(K)$ frame attains test perplexity $154$, against $279$ for a frame frozen at its random initialization and $267$ for the identity frame $\Omega_{ij} = I$. The learned-versus-frozen contrast is exactly parameter-matched and architecture-identical (both allocate the same $15{,}152{,}998$ parameters and the same gauge frame; only the frame learning rate differs), so the $45\%$ perplexity reduction it records measures the value of learning the transport geometry rather than added capacity. A frozen random frame is worse than the identity ($279$ versus $267$), showing that an arbitrary transport actively harms inference and that the benefit is specific to the learned frame. The identity arm additionally removes the learned positional gauge and so confounds transport with positional structure; it is reported only as a secondary comparison, with the parameter-matched learned-versus-frozen contrast as the controlled test. This is the gauge ablation deferred above, and it locates the advantage in the gauge-transport geometry rather than in the diagonal-covariance beliefs or the KL-divergence attention alone.

The second ablation toggles the canonical attention-entropy term of Eq.~\eqref{eq:F_align_canonical_tau}, comparing the envelope (canonical) gradient of Section~\ref{sec:final_free_energy} against the entropy-suppressed surrogate, with both arms evaluated through automatic differentiation on the same belief trajectory so that the contrast isolates the entropy term. Equation~\eqref{eq:autograd_envelope_gap} predicts that the two gradients differ by the covariance $-\tau^{-1}\mathrm{Cov}_{\beta^*}(E_{ij}, \partial E_{ij}/\partial x)$, which vanishes in the high-temperature limit $\tau \to \infty$. The measurements follow this prediction: at the lower-temperature, sharper-attention setting $\kappa = 0.25$ the surrogate is $14\%$ worse than the canonical gradient ($167$ versus $146$ test perplexity), whereas at the higher-temperature, more diffuse setting $\kappa = 1$ the two agree to within the noise floor ($146$ versus $144$). The canonical attention-entropy term is therefore load-bearing where the covariance gap of Eq.~\eqref{eq:autograd_envelope_gap} is large and negligible where it vanishes, the first empirical confirmation of the envelope-versus-surrogate distinction.
