...
\(Y(c),Y_D,\mathsf Y_D\) &
\(Y(c)\in\prod_i(\mathsf K_{i,c}\times\mathsf M_{i,c})\); \(Y_D\) valued in \(\mathsf Y_D\) &
Population sample at one context, finite-design random element, and its finite product sample space.\\
\(o,\mathsf O_D\) &
\(o\in\mathsf O_D=\prod_{a,i}\mathsf O_{i,c_a}\) &
Finite-design observation and observation sample space.\\
\(X,\mathsf X\) &
\(X\in\mathsf X\) &
Structural data conditioned upon by the state model, possibly including \(D\), supports, graph, deterministic frames or links, and fixed parameters.\\
\(P_\theta,p_\theta\) &
\(P_\theta:\mathsf X\to\mathcal P(\mathsf O_D\times\mathsf Y_D)\) &
Normalized generative probability kernel and, when dominated, its density. No factor reads a live recognition marginal.\\
\(Q_X(dY\mid o)\) &
\(Q:(\mathsf O_D\times\mathsf X)\to\mathcal P(\mathsf Y_D)\) &
Normalized, generally correlated recognition kernel whose coordinate pushforwards give \(q_i(c_a)\) and \(s_i(c_a)\).\\
\(P_0(dX),R(dX\mid o)\) &
\(P_0\in\mathcal P(\mathsf X)\); \(R:\mathsf O_D\to\mathcal P(\mathsf X)\), with \(R(\mathord\cdot\mid o)\ll P_0\) where required &
Configuration prior and configuration recognition law in the optional hierarchical tier.\\
\(\rho_0(dX),P_{\mathcal F}(dX)\) &
\(\rho_0,P_{\mathcal F}\in\mathcal P(\mathsf X)\) when the partition function is finite &
