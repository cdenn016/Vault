...
\end{equation}
\status{DEFINITION} These formulas type the action; closure of a declared
fiber under it is a separate hypothesis.
...
\status{ESTABLISHED} Hence density charts cannot be inferred from the general
law fiber.
...
framework for this tier \citep{AyJostLeSchwachhoefer2018}. Stratified and
infinite-dimensional tiers require their own tangent and integration
theories; no universal global Fisher metric or natural chart is claimed.
\status{HYPOTHESIS}\status{NOT-CLAIMED}
...
\status{ESTABLISHED} This does not rule out a quotient, pin, new reference
measure, or boundary stratum; each is a different declared object.
\status{NOT-CLAIMED}
...
\end{equation}
\status{ESTABLISHED} This is the exact exponential-family divergence; in
general it is not a finite-displacement Fisher quadratic
\citep{Csiszar1967,Brown1986,Amari2016}.
