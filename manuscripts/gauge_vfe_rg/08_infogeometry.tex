...
\Cref{ch:expfamily} distinguishes general belief and model-law fibers, generative-kernel fibers, smooth statistical manifolds, regular exponential families, and the multivariate-Gaussian realization. This chapter works only in the last of those tiers. A general belief fiber need not be dominated, differentiable in quadratic mean, finite dimensional, exponential, or equipped with a nondegenerate Fisher metric. Here those extra obligations are discharged by the nondegenerate Gaussian family, and the resulting statistical manifold is equipped with the structure the renormalization chapters will use: a pair of dual coordinate systems, the Fisher matrix as the Hessian of a potential, and the invariants of that matrix under a change of local frame.

Three things come out of the exercise, and they are of three different kinds. The first is bookkeeping done correctly. The Legendre dual of the natural coordinates is the raw-moment coordinate system, not the mean-covariance one, and the Fisher matrix has visibly different blocks in the two; an identity that mixes the mean and covariance sectors is therefore meaningless until its chart is named. The second is a prohibition. Under a change of local frame the interaction precision and its Laplacian part transform by the same congruence, so the generalized eigenvalues of a regular pair are frame independent while the ordinary eigenvalues of either matrix alone are not; any criterion phrased in absolute eigenvalues of the precision is a property of the chart rather than of the law. The third is a refusal. Nothing in this chapter asserts that any flow contracts an information-geometric quantity, and Section~\ref{sec:ig-notclaimed} states exactly what would have to be supplied before such a claim could be made.
...
\end{equation}
The distinction between the precision coordinate and the canonical natural coordinate is sign-sensitive. The information-form coordinate pair $(h,J)$ ranges over
\begin{equation}
\R^n\times\Sym_{++}^n,
\label{eq:ig-information-domain}
\end{equation}
whereas the actual natural parameter $\eta=(h,\eta_2)$ paired against $T(y)=(y,yy^\top)$ ranges over
\begin{equation}
\mathsf N_\eta
=\R^n\times\Sym_{--}^n,
\qquad
\eta_2=-\tfrac12J.
\label{eq:ig-natural-domain}
\end{equation}
Both domains are open and convex and are affinely equivalent under $J\mapsto-J/2$, but they are opposite definite cones. The sufficient statistics are affinely independent, so the family is minimal; and $\mathsf A$ is finite and infinitely differentiable on the natural domain, so the family is regular and steep in the sense of the standard exponential-family theory \citep{Brown1986}. Regularity is what makes the Legendre transform below a bijection rather than a formal manipulation, and it is an established property of this Gaussian subclass, not an assumption about the general belief fibers. \status{ESTABLISHED}
...
Four coordinate presentations on the same manifold will be named, and they must be kept apart. The \emph{canonical natural} chart is $\eta=(h,-J/2)$ as in \eqref{eq:ig-natural}, whose matrix component is negative definite. The affinely equivalent \emph{information-form} chart $(h,J)$ uses the SPD precision $J$; calling $J$ positive definite never makes $\eta_2$ positive definite. The \emph{expectation} chart is $\tau=(\mu,M)$ as in \eqref{eq:ig-expectation}; it is the Legendre dual of the canonical natural chart, and it is the chart in which \eqref{eq:ig-dual-inverse} holds as stated. The \emph{moment} chart is $(\mu,C)$, the mean together with the centered covariance. The moment chart is a perfectly good chart, and it is the one used for sampling, for covariance transport, and for every calculation on the cone of positive definite matrices; it is simply not the dual of the natural chart, because the dual second coordinate is the raw second moment $M$ and not the centered one $C=M-\mu\mu^\top$. Declaring these chart names is a convention, and nothing is proved by the declaration. \status{DEFINITION}
...
\medskip
\noindent\textbf{Proposition 9.1 (Fisher metric in the moment chart).}
...
\medskip
\noindent\textbf{Proposition 9.2 (Fisher metric in the expectation chart).}
...
\medskip
\noindent\textbf{Corollary 9.3 (size of the discrepancy, and the exact equality case).}
...
The first symbolic route is independent of Proposition~9.1: the Hessian of \eqref{eq:ig-lognormalizer} was formed by computer algebra in the natural chart $(h,\operatorname{svec}(-J/2))$ with the Frobenius-paired symmetric coordinates, evaluated at exact rational $(\mu,\Lambda)$, inverted exactly as in \eqref{eq:ig-dual-inverse}, and its blocks compared with \eqref{eq:ig-meanblock} and \eqref{eq:ig-crossblock}. At dimensions $n=1,2,3$ the difference was the exact zero matrix in both blocks, and at $n=2$ the cross block became exactly zero when $\mu=0$ was substituted and was nonzero otherwise, as Proposition~9.2 requires. The second symbolic route carries general symbols: at dimensions $n=2$ and $n=3$, with $\mu$ a vector of free symbols and $\Lambda$ a general symmetric matrix of free symbols, the pullback of \eqref{eq:ig-moment-metric} along \eqref{eq:ig-chartjacobian} was simplified to zero difference against all three blocks of Proposition~9.2. Those two runs are proofs at those dimensions, not samples.

The deterministic numerical route \texttt{CHK-IG-EXPECTATION-METRIC} uses the closed-form Gaussian relative entropy and its Fisher quadratic limit in six directions at a fully specified base point. It compares the expectation-chart block with Equation~\eqref{eq:ig-meanblock}, uses the moment-chart precision as a negative control against chart conflation, and checks the predicted generalized spectrum. It replaces the earlier incompletely specified score-covariance run. The computation is reproducible corroboration, not a proof. \status{NUMERICAL}
...
\medskip
\noindent\textbf{Proposition 9.4 (pullback versus pushforward).}
...
\medskip
\noindent\textbf{Proposition 9.5 (frame invariance of the generalized spectrum).}
...
\]
so the full-space determinant does not define an $n$-element generalized spectrum. A quotient, pin, or mass repair must first be declared and the resulting pencil proved regular before the conclusions below can be invoked. Thus Propositions~9.5--9.7 apply to the Gaussian probability interior $\Lambda\succ0$ (or to a separately proved regular repair), not automatically to an operator-layer boundary point. That qualification is the pencil-level instance of the hierarchy in \Cref{ch:expfamily}.

\medskip
\noindent\textbf{Proposition 9.6 (localization and endpoints).}
...
\medskip
\noindent\textbf{Proposition 9.7 (frame dependence of absolute spectra and of subspace determinants).}
...
\midrule
Sec.~\ref{sec:ig-expfam} & Gaussian regular, minimal, steep exponential family; $\eta_2=-J/2\in\Sym_{--}^n$ while $J\in\Sym_{++}^n$; dual pair $\eta\leftrightarrow(\mu,M)$ & \status{ESTABLISHED} & Gaussian subclass only; no Fisher structure inferred for a general belief fiber \\
Sec.~\ref{sec:ig-expfam} & Canonical natural, information-form, expectation, and moment chart names; Frobenius-paired symmetric coordinates & \status{DEFINITION} & Nothing proved; a naming convention \\
Prop.~9.1 & Moment-chart metric \eqref{eq:ig-moment-metric} is block diagonal with mean block $\Lambda$ & \status{ESTABLISHED} & none \\
Prop.~9.2 & Expectation-chart mean block \eqref{eq:ig-meanblock}; nonzero cross block \eqref{eq:ig-crossblock} & \status{ESTABLISHED} & none \\
Cor.~9.3 & $g_{[\mu\mu]}\succeq\Lambda$ with ratio spectrum $\{1+\mu^\top\Lambda\mu,1+2\mu^\top\Lambda\mu\}$; equality iff $\mu=0$ & \status{ESTABLISHED} & none \\
...
Sec.~\ref{sec:ig-chart} & Deterministic check \texttt{CHK-IG-EXPECTATION-METRIC} of Propositions 9.1 and 9.2 & \status{NUMERICAL} & Current reproducible corroboration; proofs are in the text \\
...
Prop.~9.4 & Restriction dominates marginal, Eq.~\eqref{eq:ig-restriction-marginal}; equality iff $\Lambda$-orthogonal & \status{ESTABLISHED} & none \\
...
Sec.~\ref{sec:ig-gauge} & Block-diagonal reframing; congruence acts on $L$ and $A$ separately & \status{HYPOTHESIS} & Excludes agent-mixing coordinate changes; used in Propositions 9.5 through 9.7 \\
Prop.~9.5 & Generalized eigenvalues of the symmetric-definite pair $(L,\Lambda)$ are frame independent & \status{ESTABLISHED} & Requires $\Lambda\succ0$ or a separately proved regular repair; a common singular pencil can have identically zero determinant \\
Prop.~9.6 & Generalized spectrum lies in $[0,1]$; $0$ on $\ker L$, $1$ on $\ker A$ & \status{ESTABLISHED} & none \\
Prop.~9.7 & Absolute spectra and subspace log determinants are frame dependent, Eq.~\eqref{eq:ig-logdet-law} & \status{ESTABLISHED} & none \\
Sec.~\ref{sec:ig-gauge} & Deterministic check \texttt{CHK-GENERALIZED-SPECTRUM} over twelve block congruences, with moving ordinary spectrum as control & \status{NUMERICAL} & Current reproducible corroboration; proofs are in the text \\
