...
Two distinct freedoms act on these data and are kept apart below. Agent $i$ may re-choose its frame within the fiber over $c$; since every point of that fiber is $\sigma_0(c)\cdot g$ for a unique $g$, any re-choice may be written $U_i\mapsto g_iU_i$ for a smooth $g_i:\mathcal C_i\to G$, and the represented fiber coordinates relabel by $\rho_k(g_i)$ and $\rho_m(g_i)$. Separately, the reference itself may be changed, $\sigma_0\mapsto\sigma_0\cdot h$ for a smooth $h:\mathcal C_0\to G$, which sends every coordinate simultaneously to $U_i\mapsto h^{-1}U_i$. The first is a per-agent freedom and is the vertex gauge transformation of Section~\ref{sec:geo-regime-one}; the second is a single global relabeling of the trivialization.

The supports $\mathcal C_i$ are indexed by agents, which is a modeling choice and not forced: nothing prevents two agents from sharing a support, and nothing requires the family $\{\mathcal C_i\}$ to be a good cover. What the trivialized regime requires in addition is that every frame be referable to the single reference section $\sigma_0$. Section~\ref{sec:geo-regime-one} records that requirement as an explicit hypothesis and says what it excludes, because it is the substantive content of what is later called flatness. Section~\ref{sec:geo-cech} then removes it: the supports are an open cover, the comparisons are a nonabelian \v{C}ech cochain on its nerve, and the requirement is proved equivalent to that cochain being a coboundary and to $P\vert_{\mathcal C_0}$ being trivial. The general case, in which only local sections exist, is developed there rather than declined.

\section{Two represented channels from one group element}
\label{sec:geo-associated}

Probability enters through associated bundles. Let $\mathsf K$ and $\mathsf M$ denote the state and model sample fibers in a local trivialization, and let $\mathcal B_{\mathrm{state}}$ and $\mathcal B_{\mathrm{model}}$ be statistical manifolds whose points are probability measures on $\mathsf K$ and on $\mathsf M$ respectively \citep{Amari2016,AmariNagaoka2000}. A point of $\mathcal B_{\mathrm{state}}$ is a law, not a draw from a law; the distinction is used in Section~\ref{sec:geo-agents} to keep agents apart from population samples.
...
Measurability suffices for \eqref{eq:geo-pushforward-actions} but not for a smooth associated bundle, so the additional requirement is stated rather than assumed silently.

\medskip
\noindent\textbf{Hypothesis 2.3 (Smooth, family-preserving represented actions).} The chosen statistical families are closed under their represented pushforwards, and the induced maps
\begin{equation}
...
\end{equation}
are smooth in the selected statistical-manifold coordinates. \status{HYPOTHESIS}
\medskip

This is a choice, and it excludes families that a represented action carries outside themselves. It is used only to make the associated-bundle construction smooth rather than merely set theoretic, and it is discharged for the case this document actually uses: Proposition 2.6 verifies closure and smoothness for nondegenerate Gaussian families under invertible linear represented actions, so for the Gaussian realization the hypothesis is a theorem. For a general family it remains a restriction.

\medskip
\noindent\textbf{Definition 2.4 (Associated statistical bundles).} With the actions of Hypothesis 2.3 written $\rho_{\mathrm{state}}$ and $\rho_{\mathrm{model}}$, set
\begin{equation}
...
The two bundles are built over one principal bundle, from two representations of one group. When a frame change $g_i\in G$ is applied at agent $i$, both channels move, by
\begin{equation}
g_i^{k}=\rho_k(g_i),
\qquad
g_i^{m}=\rho_m(g_i).
...
\end{equation}
where $U_i$ is the frame field \eqref{eq:geo-frame-field} and
\begin{equation}
...
\end{equation}
whereas under a change of reference $\sigma_0\mapsto\sigma_0\cdot h$ it transforms by the single conjugation $\Omega_{ij}\mapsto h^{-1}\Omega_{ij}h$. \status{ESTABLISHED}
\medskip

\noindent\emph{Proof.} The three identities in \eqref{eq:geo-cocycle} are immediate from \eqref{eq:geo-omega-definition}, the third by cancellation of $U_j(c)^{-1}U_j(c)$; being of the form $U_iU_j^{-1}$ is exactly the statement that the family is a coboundary. Equation~\eqref{eq:geo-regime-one-gauge-law} follows by substituting $U_i'=g_iU_i$ into \eqref{eq:geo-omega-definition}, and the reference-change law follows from $U_i\mapsto h^{-1}U_i$, which gives $(h^{-1}U_i)(h^{-1}U_j)^{-1}=h^{-1}U_iU_j^{-1}h$. \hfill$\square$
...
Hypothesis 2.10 is therefore not an auxiliary convenience layered on top of a topological condition. It is the topological condition, in coordinates. Saying that every frame is referable to one reference section, saying that the comparison cochain is a coboundary, and saying that $P\vert_{\mathcal C_0}$ is trivial are three phrasings of one hypothesis, and the substantive content of what Section~\ref{sec:geo-flatness} calls flatness is that hypothesis and nothing else.
...
\medskip
\noindent\textbf{Proposition 2.22 (What the obstruction is, and what it is not).} Let $\mathcal G$ denote the sheaf of smooth $G$-valued functions on $\mathcal C_0$. Then the following two statements hold and are distinct.
...
Second, what \eqref{eq:geo-cech-class} obstructs is a global trivialization, and not the existence of global sections of the associated statistical bundles. For the Gaussian realization of Proposition 2.6 the fiber $\mathcal B^{\mathrm G}_{\mathrm{state}}\cong\R^{K}\times\Sym^{K}_{++}$ is contractible, and $\mathcal C_0$ is an open subset of a second countable Hausdorff manifold, hence paracompact and of the homotopy type of a CW complex; the successive obstructions to extending a cross-section over skeleta lie in $H^{n+1}(\mathcal C_0;\pi_n(\mathcal B^{\mathrm G}_{\mathrm{state}}))$, with local coefficients where the base is not simply connected, and all vanish because every homotopy group of a contractible space is trivial; so $\mathcal E_{\mathrm{state}}$ admits a global continuous cross-section whatever the class \eqref{eq:geo-cech-class} \citep{Steenrod1951}. \status{ESTABLISHED}
\medskip

Both statements are established: the first by the standard classification of bundles trivialized over a cover, cited above rather than reproved here, and the second by the obstruction count given with it. The second is included because the opposite claim is easy to make and is false. What fails in the general regime is that the fibers of $\mathcal E_{\mathrm{state}}$ cannot be identified with one model fiber in a way valid over all of $\mathcal C_0$, so that two agents' laws at a shared context have no common coordinates in which to be subtracted; that failure is the content of the nontrivial class. It is not the claim that no distribution-valued field exists over $\mathcal C_0$, which for a contractible fiber is refuted by the obstruction count just given. The smooth refinement of that count is standard and is not used anywhere below, so nothing here rests on it.
...
Part (c) is a limitation of the criterion and is stated as one rather than left for a reader to discover. Where the comparison assignment comes from a \v{C}ech cocycle evaluated at one context, the cocycle identity already supplies the coboundary, so the criterion holds automatically and detects nothing. Its content lies where Proposition 2.18 does not reach, and that is Regime~II: there the links are declared independently of the frames, no triple-overlap identity constrains them, and Proposition 2.13 exhibits an assignment whose holonomy is not the identity. Extending the Regime-I products to comparisons taken at different contexts would not evade this, because such a product is not defined by the data of Section~\ref{sec:geo-cech} at all; forming it requires the principal connection of Section~\ref{sec:geo-three-transports} to move between fibers, which is the separate object whose relation to link holonomy is Open 2.16. The genuine bundle obstruction \eqref{eq:geo-cech-class} is of a third kind again: it is a cohomological invariant of the cover, and no finite product of comparisons computes it. The three are distinct, and no result here treats any one of them as evidence about another.

Two consequences orient the rest of the development. The flat regime is exactly the case in which the entire population $V$ is a single trivializing subfamily, by Proposition 2.24 with $F=V$ together with Proposition 2.19; so flatness is not an extra condition alongside the trivializing property but the maximal instance of it. And in the general regime the population decomposes into trivializing subfamilies that are not canonical: the family of them is downward closed with maximal elements, by Proposition 2.27(d), but a choice of which maximal elements to use is a declaration, not a computation. Chapter~\ref{ch:coarsegraining} coarsens along such a choice, and the reader should carry Proposition 2.24 there as the statement of what a cluster must satisfy for its internal edges to be removable.
...
Section~\ref{sec:geo-three-transports} introduced the principal connection $\boldsymbol\omega\in\Omega^{1}(P,\mathfrak g)$ abstractly, as an object the population's frames do not by themselves supply. This section supplies one canonically, built from nothing beyond the frames $u_i$ of Definition 2.2 and a partition of unity, and states exactly when the result is flat. It does not close Open 2.16: the connection constructed here is a functional of the frames alone and carries no reference to the declared graph $\mathfrak I$ or its links $L_{ij}$, so nothing below relates its holonomy to $\operatorname{Hol}(\gamma)$.
...
\medskip
\noindent\textbf{Proposition 2.31 (Induced global connection, and when it is flat).} Principal connections on $P\vert_{\mathcal C_0}$ form an affine space modeled on the vector space of horizontal, $\operatorname{Ad}$-equivariant $\mathfrak g$-valued $1$-forms on $P\vert_{\mathcal C_0}$: the difference of two connection forms is exactly such a tensorial form, and adding such a form to a connection form gives another connection form \citep{Kobayashi1963}. Let $\{\chi_i\}$ be a smooth partition of unity subordinate to the cover $\{\mathcal C_i\}$ of $\mathcal C_0$, which exists because $\mathcal C_0$, an open subset of a second countable Hausdorff manifold, is paracompact. Then
\begin{equation}
...
The hypothesis of Proposition 2.31 is a statement about how the frames vary relative to one another on overlaps, not about which regime declares the links, and it can fail, generically does fail, even when Hypothesis 2.14 holds. The flat link regime fixes $L_{ij}=\Omega_{ij}=U_iU_j^{-1}$ at each context but places no constraint on how $U_i$ and $U_j$ vary as $c$ moves, so $t_{ij}$ is locally constant only for special choices of frame. The base holonomy of \eqref{eq:geo-induced-connection} is therefore a genuinely separate question from the triviality Proposition 2.11 establishes, which is a statement at one context and does not see how the frames vary over $\mathcal C$ at all.

\medskip
\noindent\textbf{Open 2.32 (Converse of the flatness criterion).} Whether disagreement of the local connections, $\omega_i\neq\omega_j$ somewhere on some overlap, forces the induced connection \eqref{eq:geo-induced-connection} to have nonvanishing curvature for \emph{every} choice of subordinate partition of unity is not settled here. \status{OPEN}
\medskip

Proposition 2.31 supplies only a sufficient condition; nothing above rules out a fortuitous choice of $\{\chi_i\}$ under which the curvature contributions of disagreeing patches cancel rather than accumulate. What would settle it is either such a cancelling example, exhibiting the converse as false, or a bound on the curvature of \eqref{eq:geo-induced-connection} in terms of the local disagreements that holds uniformly over partitions of unity. This restates Open 2.16 by inversion: that question asked whether a declared discrete holonomy could be realized as a base holonomy, and this one asks whether the absence of local disagreement is necessary for the base holonomy to vanish. Both remain open for the same reason, that the discrete and the base constructions are built from different data and no bridge between them has been supplied.

The chapter now has three constructions that are each sometimes shortened to the single word "holonomy," and keeping them apart is worth restating in one place now that all three exist. The first is the ordered product of Regime-I comparisons around a closed walk evaluated at one context, Proposition 2.11: it is always the identity, by the cocycle telescoping alone, regardless of how the frames vary over $\mathcal C$, and it carries no information about the population and none about the base. The second is the holonomy of a comparison assignment on a declared graph, Definition 2.23 and Proposition 2.24; in the Regime-II instance it is the object the coarse-graining criterion of Chapter~\ref{ch:coarsegraining} reads, and Proposition 2.13 exhibits it nontrivial. The third is the curvature-governed holonomy of the induced connection of this section, evaluated around a closed curve in $\mathcal C$ rather than around a closed walk in a population graph; Proposition 2.31 shows it can be nontrivial even under Hypothesis 2.14, precisely because it depends on how the frames vary over $\mathcal C$ rather than on their values at isolated contexts, which is the object Open 2.16 was reaching for and which Open 2.32 shows still has not been related to the second. No result above treats any one of these three as evidence about another, and Open 2.16 and Open 2.32 are exactly the two places where a bridge between them was sought and not found.
...
\medskip
\noindent\textbf{Hypothesis 2.14 (Flat link regime).} For every pair of interacting agents the link is the Regime-I comparison,
\begin{equation}
...
In the vocabulary of Section~\ref{sec:geo-trivializing} the hypothesis has a compact reading: it says that the entire population is a single trivializing subfamily, since by Proposition 2.24 the coboundary form $L_{ij}=U_iU_j^{-1}$ on all of $V$ is equivalent to triviality of every closed-walk holonomy, and by Proposition 2.19 that is in turn equivalent to Hypothesis 2.10. Flatness is therefore the maximal case of a graded property rather than a separate condition, and the graded version is what remains when it is dropped. Under the general regime of Definition 2.20 the population still decomposes into trivializing subfamilies, each of which admits its own common trivialization by Proposition 2.24, and the following identity holds internally to each of them with $U_i$ replaced by the $V_i$ that the criterion supplies.
...
Whether an analogue of that closure holds for a genuinely non-flat connection Laplacian is open, both in this development and in the literature on aggregation-based coarse operators that Part~III draws on. It is raised where it is used rather than here, since stating it requires the interaction family that Part~II declares.
...
2.3 & Smooth, family-preserving represented actions & \textsc{hypothesis} & A choice; discharged for Gaussians by 2.6, open for general families \\
2.4 & Associated statistical bundles & \textsc{definition} & Nothing proved \\
...
2.7 & Agent as section-bearing object & \textsc{definition} & Nothing proved \\
...
2.22 & Obstruction is a class in nonabelian $\check H^1$, a pointed set; it does not obstruct sections of $\mathcal E_{\mathrm{state}}$ & \textsc{established} & Standard, cited \citep{Serre1997Galois,Husemoller1994,Steenrod1951}; the section-existence clause is the vanishing obstruction count for a contractible fiber, stated for continuous sections only \\
...
\begin{sciencetable}[htbp]{Status register for Chapter~\ref{ch:geometry}, continued: the induced connection of Section~\ref{sec:geo-induced-connection}. Together with Tables~\ref{tab:geo-register} and~\ref{tab:geo-register-topology} this lists every numbered result of the chapter exactly once.}
\begin{tabular}{@{}p{0.13\textwidth}p{0.33\textwidth}p{0.13\textwidth}p{0.32\textwidth}@{}}
\toprule
\textbf{Result} & \textbf{Content} & \textbf{Status} & \textbf{What is owed} \\
\midrule
...
2.31 & Induced global connection by partition of unity; flat when local forms agree on overlaps & \textsc{established} & Proved in text; the sufficient condition is stated precisely, and holds independently of Hypothesis 2.14 \\
2.32 & Converse of the flatness criterion & \textsc{open} & Needs either a cancelling example under some subordinate partition of unity or a partition-of-unity-uniform lower bound on curvature in terms of local disagreement; restates Open 2.16 by inversion \\
\bottomrule
\end{tabular}
