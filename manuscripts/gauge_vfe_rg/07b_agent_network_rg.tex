...
\section{Dynamic closure lives on path space}
\label{sec:rg-path-space}

For a finite time interval, apply the timewise coarse channel to the full joint
path law.  \Cref{thm:rg-exact-coarse-vfe} then applies on path space without
change.  The resulting coarse path law is normalized and gauge covariant when
spatial and temporal transports are included.  It need not be first-order
Markov.  \status{ESTABLISHED}
