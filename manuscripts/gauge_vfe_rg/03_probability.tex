...
\medskip
\noindent\textbf{Definition 3.1 (Finite design and incidence).} A \emph{design} is a finite set of distinct contexts
\begin{equation}
D=\{c_a\}_{a=1}^{M}\subseteq\mathcal C,
\label{eq:prob-design}
\end{equation}
indexed by $a\in\{1,\dots,M\}$. The \emph{incidence set} is $\mathfrak A=\{(i,a):c_a\in\mathcal C_i\}$, where $\mathcal C_i$ is the support of agent $i$ from Definition 2.2. For the reference presentation $D\subseteq\bigcap_{i\in V}\mathcal C_i$, so $\mathfrak A=V\times\{1,\dots,M\}$ is rectangular and every listed agent is defined at every design point. \status{DEFINITION}
\medskip
...
\medskip
\noindent\textbf{Definition 3.2 (Sample spaces and product sigma-algebras).} At each admissible $(i,a)$ declare measurable state, model, and observation fibers $(\mathsf K_{i,a},\mathscr K_{i,a})$, $(\mathsf M_{i,a},\mathscr M_{i,a})$, and $(\mathsf O_{i,a},\mathscr O_{i,a})$. The finite latent and observation spaces are
\begin{align}
...
\medskip
\noindent\textbf{Definition 3.3 (Structural configuration and kernel signatures).} Fix a measurable configuration space $(\mathsf X,\mathscr X)$. Every $X\in\mathsf X$ uses the same design $D$, the same agent index set $V$, the same incidence set $\mathfrak A$, and the same measurable identifications of the fibers in \eqref{eq:prob-latent-space} and \eqref{eq:prob-observation-space}; therefore $(\mathsf Y_D,\mathscr Y_D)$ and $(\mathsf O_D,\mathscr O_D)$ do not vary with $X$. A configuration may carry the finite interaction complex $\mathfrak I$ of Definition 2.12 with its edge set, the frames $U_i$ or the links $L_{ij}$, and the parameters of the observation and transition mechanisms. The generative and recognition objects have the signatures
\begin{align}
...
Mixed coordinates are the case where a declaration is genuinely needed rather than conventional, and the following statement supplies it.

\medskip
\noindent\textbf{Proposition 3.5 (Dominating measure for a mixed coordinate).} Let $\mu$ be a Borel probability measure on $\R$ whose Lebesgue decomposition has no singular continuous part, so that $\mu=\mu_{\mathrm{ac}}+\mu_{\mathrm d}$ with $\mu_{\mathrm{ac}}\ll\lambda$ and $\mu_{\mathrm d}$ carried by a countable set $A$. Put
\begin{equation}
...
\noindent\emph{Proof.} For sigma-finiteness, write $\R=\bigl(\bigcup_{n\geq1}([-n,n]\setminus A)\bigr)\cup\bigcup_{x\in A}\{x\}$, a countable union; $\nu([-n,n]\setminus A)=\lambda([-n,n]\setminus A)\leq2n<\infty$ and $\nu(\{x\})=\lambda(\{x\})+1=1<\infty$. For absolute continuity, let $\nu(B)=0$. Then $\lambda(B)=0$ and $B\cap A=\emptyset$, since a single atom of $A$ inside $B$ would contribute $1$. Hence $\mu_{\mathrm{ac}}(B)=0$ because $\mu_{\mathrm{ac}}\ll\lambda$, and $\mu_{\mathrm d}(B)=\mu_{\mathrm d}(B\cap A)=0$ because $\mu_{\mathrm d}$ is carried by $A$. Therefore $\mu(B)=0$. \hfill$\square$

The hypothesis excluding a singular continuous part is a real restriction and is stated rather than assumed. A coordinate whose law carries a singular continuous component, of which the Cantor distribution is the standard example, is not dominated by \eqref{eq:prob-mixed-reference}, and this development supplies no generic reference measure for it. Such a coordinate would require its own declaration, and the declaration would have to be made before any density on that coordinate is written.

\section{Kernels first, densities second}
\label{sec:prob-kernels}

\medskip
\noindent\textbf{Definition 3.6 (Normalized kernels; densities as derivatives).} The objects \eqref{eq:prob-generative-signature} and \eqref{eq:prob-recognition-signature} are probability kernels, hence normalized by type. When $P_\theta(\cdot\given X)\ll\nu^{O}_{D}\otimes\nu^{Y}_{D}$, its density is the Radon-Nikodym derivative $p_\theta(o,y\given X)$ determined by
\begin{equation}
...
The order in Definition 3.6 is the content. A normalized kernel is declared, and only then is a density extracted; the reverse order, in which a nonnegative function is written down and a normalizer is invoked afterward, is a different construction whose normalizer must be shown finite before anything it appears in denotes a probability. This document uses the first order everywhere, and where a later chapter writes an unnormalized energy it says so and exhibits the normalizer.

The same order is what makes the group action of Chapter~\ref{ch:geometry} well typed. The action \eqref{eq:geo-pushforward-actions} is defined on measures. A frame change that does not preserve the reference measure changes the density and the base-measure Jacobian together, and only their combination is invariant, so a rule that multiplies a matrix into a density and stops is not the represented action unless the Jacobian is identically one.

\medskip
\noindent\textbf{Proposition 3.7 (Kernel integration preserves measurability).} Let $\kappa$ be a probability kernel from $(\mathsf E,\mathscr E)$ to $(\mathsf F,\mathscr F)$ and let $f:\mathsf E\times\mathsf F\to[0,\infty]$ be $\mathscr E\otimes\mathscr F$-measurable. Then
\begin{equation}
...
\medskip
\noindent\textbf{Definition 3.8 (Standing convention on logarithmic density ratios).} Wherever this document writes a logarithmic density ratio, or an expectation of one, the measure appearing in the numerator is absolutely continuous with respect to the measure appearing in the denominator. Relative entropy is understood in the measure-theoretic sense,
\begin{equation}
\KL(Q\Vert P)=
\begin{cases}
...
\medskip
\noindent\textbf{Proposition 3.9 (Density criterion for absolute continuity).} Let $Q$ and $P$ be measures on a common measurable space with $Q\ll\nu$ and $P\ll\nu$ for a sigma-finite $\nu$, with densities $q$ and $p$. Write $Z=\{p=0\}$. Then
\begin{equation}
Q\ll P
\iff
Q(Z)=0
...
The consequence used later is direct. A candidate recognition law that places mass where the posterior places none fails the criterion, so by \eqref{eq:prob-kl-definition} its relative entropy to the posterior is $+\infty$, and by the exact bound of Chapter~\ref{ch:elbo} its free energy is $+\infty$ as well. Degenerate laws supported on a proper subspace, when the posterior is nondegenerate, are exactly of this kind: the subspace is a null set of the reference measure and hence carries no posterior density, so it lies inside $Z$ and receives full mass under the candidate law, and the criterion fails maximally. The specific instance, in which an identification of latent coordinates across a cluster is shown to be a generative construction rather than a restriction on the recognition law, is developed in Part~II; what is established here is only the general criterion it invokes.

\medskip
\noindent\textbf{Hypothesis 3.10 (Support and log-integrability).} For the observed $o$ and the fixed $X$,
\begin{equation}
0<p_\theta(o\given X)<\infty,
\qquad
Q_X(\cdot\given o)\ll P_\theta(\cdot\given o,X),
...
These are choices, and each excludes something identifiable. The lower bound in \eqref{eq:prob-support-condition} excludes observations the model deems impossible, for which no bound on the log evidence exists because the log evidence is $-\infty$. The absolute-continuity requirement excludes exactly the degenerate laws just described. Condition \eqref{eq:prob-integrability} excludes recognition laws whose tails make the two expectations individually infinite even when their difference would formally cancel, and it is what permits the decomposition in Chapter~\ref{ch:elbo} to be rearranged term by term. Where a later result tolerates an extended-real relative entropy, the relaxation is stated at that point rather than absorbed into the notation here.

\section{Recognition marginals and what they fail to determine}
\label{sec:prob-marginals}

\medskip
\noindent\textbf{Definition 3.11 (Coordinate marginals of the recognition law).} With $\operatorname{pr}^{k}_{i,a}$ and $\operatorname{pr}^{m}_{i,a}$ the coordinate projections out of $\mathsf Y_D$, define the marginalization maps $\operatorname{Marg}^{k}_{i,a}(R)=(\operatorname{pr}^{k}_{i,a})_{\#}R$ and $\operatorname{Marg}^{m}_{i,a}(R)=(\operatorname{pr}^{m}_{i,a})_{\#}R$ on $\mathcal P(\mathsf Y_D,\mathscr Y_D)$, and set
\begin{equation}
...
The recognition law is not required to factor across agents, design points, or channels, and in the development that follows it does not. The marginals \eqref{eq:prob-recognition-marginals} are therefore a lossy summary, and the next statement says how lossy, by exhibiting the loss rather than asserting it.

\medskip
\noindent\textbf{Proposition 3.12 (Coordinate marginals do not determine the joint).} There exist distinct probability measures on $\R^{2}$ with identical coordinate marginals. Consequently the family $\{q^{o,X}_{i,a},s^{o,X}_{i,a}\}$ does not determine $Q_X(\cdot\given o)$, and the correlated finite law is primitive at the inference level. \status{ESTABLISHED}
\medskip
...
\end{equation}
holds. Then (i) \eqref{eq:prob-compatibility} does not determine $Q_X(\cdot\given o)$, and (ii) \eqref{eq:prob-compatibility} does not determine the sections $q^{o,X}_i$ and $s^{o,X}_i$. \status{ESTABLISHED}
\medskip

\noindent\emph{Proof.} (i) By Proposition 3.12 there are distinct laws in $\mathcal P(\mathsf Y_D,\mathscr Y_D)$ with the same coordinate marginals, and \eqref{eq:prob-compatibility} constrains $Q_X$ only through those marginals; both such laws satisfy it against the same sections. (ii) Fix a support $\mathcal C_i$ of positive dimension. Since $D$ is finite and $\mathcal C_i$ is open and nonempty, $\mathcal C_i\setminus D$ is nonempty and open, so there is a point $c^{\star}\in\mathcal C_i\setminus D$ and a smooth bump function $\chi:\mathcal C_i\to[0,1]$ with $\chi(c^{\star})=1$ and support contained in a neighborhood of $c^{\star}$ disjoint from the finite set $D$. In the Gaussian realization, where the state fiber is coordinatized by $(\mu,\Sigma)$, the section obtained from $q^{o,X}_i$ by replacing $\mu_i(c)$ with $\mu_i(c)+\chi(c)v$ for any fixed $v\neq0$ is a smooth section of the same bundle, differs from $q^{o,X}_i$ at $c^{\star}$, and agrees with it on all of $D$; it therefore satisfies \eqref{eq:prob-compatibility} against the same marginals. \hfill$\square$

\medskip
\noindent\textbf{Hypothesis 3.14 (Sampling and marginalization are compatible).} Where a result requires the continuum sections and the finite recognition law to describe the same coordinate laws, relation \eqref{eq:prob-compatibility} is adopted. \status{HYPOTHESIS}
\medskip

This is a choice and is named as one. It excludes configurations in which an agent's continuum section and the finite law disagree at a design point, which is a genuine possibility because the two objects are declared independently. It is invoked only where a statement about the finite law is transferred to the geometric layer, or the reverse; a result stated entirely about the finite construction does not need it. Proposition 3.13 records that adopting the hypothesis buys no determination in either direction, so it can never be used to construct one object from the other.

\section{What the finite construction does not supply}
\label{sec:prob-continuum}

\medskip
\noindent\textbf{Open 3.15 (Continuum probabilistic theory).} The finite construction supplies probability spaces and kernels associated with the declared design $D$. It supplies no probability measure on a space of sections over $\mathcal C$, no unique extension of the finite marginals to such a space, and no continuum limit. Whether one exists for this construction is not settled here. \status{OPEN}
\medskip
...
\label{sec:prob-register}

\begin{sciencetable}[htbp]{Status register for Chapter~\ref{ch:probability}. Every numbered result of this chapter appears exactly once. For anything not \textsc{established}, the final column names what is owed.}
...
\midrule
3.1 & Finite design and incidence set & \textsc{definition} & Nothing proved; fixes a type \\
3.2 & Sample spaces and product sigma-algebras & \textsc{definition} & Nothing proved \\
3.3 & Configuration space and kernel signatures & \textsc{definition} & Nothing proved; the prohibition it types is used in Chapters~\ref{ch:generative} and \ref{ch:obstructions} \\
3.4 & Declared reference measures & \textsc{definition} & Nothing proved; sigma-finiteness required of every declared factor \\
3.5 & Dominating measure for a mixed coordinate & \textsc{established} & Proved in text; excludes singular continuous components, which need their own declaration \\
3.6 & Normalized kernels first, densities second & \textsc{definition} & Nothing proved \\
3.7 & Kernel integration preserves measurability & \textsc{established} & Proved in text; standard, see \citet{Kallenberg2021} \\
3.8 & Standing absolute-continuity convention & \textsc{definition} & Nothing proved; assigns $+\infty$ where the relation fails \\
3.9 & Density criterion for absolute continuity & \textsc{established} & Proved in text \\
3.10 & Support and log-integrability conditions & \textsc{hypothesis} & A choice; excludes impossible observations, degenerate recognition laws, and non-integrable tails; used in Chapter~\ref{ch:elbo} \\
3.11 & Coordinate marginals of the recognition law & \textsc{definition} & Nothing proved \\
3.12 & Marginals do not determine the joint & \textsc{established} & Proved by explicit witness \\
3.13 & Compatibility determines neither side & \textsc{established} & Proved in text, both directions \\
3.14 & Sampling and marginalization compatibility & \textsc{hypothesis} & A choice, not a consequence; invoked only when transferring statements between the geometric and finite layers \\
3.15 & Continuum probabilistic theory & \textsc{open} & Needs refining designs, a projective system and extension theorem, a topology on the section space, tightness, convergence of functionals, control of gauge and reference choices, and a normalizable continuum energy \\
\bottomrule
\end{tabular}
\label{tab:prob-register}
\end{sciencetable}
