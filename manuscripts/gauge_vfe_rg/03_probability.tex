...
\end{equation}
then $Q\not\ll P$ by the definition of absolute continuity, and \eqref{eq:prob-kl-definition} gives $\KL(Q\Vert P)=+\infty$. In particular, let $P$ be a nondegenerate Lebesgue-dominated law on $\R^d$ and let $Q$ be supported on a proper affine subspace $S$. The subspace is Lebesgue null, hence $P(S)=0$, whereas $Q(S)=1$; the direct argument \eqref{eq:prob-direct-null-set}, not Proposition~3.9, proves the claim. This distinction matters because a subspace-supported $Q$ has no Lebesgue density to which the density criterion could be applied. The specific instance, in which an identification of latent coordinates across a cluster is shown to be a generative construction rather than a restriction on the recognition law, is developed in Part~II.

\medskip
\noindent\textbf{Hypothesis 3.10 (Regular observation, support, and log-integrability).} For fixed $X$, the observation satisfies $o\in\mathsf O_{\theta,X}^{\mathrm{reg}}$ under the regular-conditional convention above, and
\begin{equation}
0<p_\theta(o\given X)<\infty,
\qquad
...
RCP convention & Standard-Borel regular conditional probability and marginal-full regular-observation set & \textsc{hypothesis} & Fixed-observation claims are marginal-a.e.; an everywhere claim requires declared pointwise versions \\
...
3.10 & Regular-observation, support, and log-integrability conditions & \textsc{hypothesis} & A choice; fixes an a.e. conditional version and excludes impossible observations, degenerate recognition laws, and non-integrable tails; used in Chapter~\ref{ch:elbo} \\
3.11 & Coordinate marginals of the recognition law & \textsc{definition} & Nothing proved \\
3.12 & In a finite latent space with at least two nondegenerate real coordinates, marginals do not determine the joint & \textsc{established} & Gaussian correlation witness; singleton-fiber counterexample records why the richness hypothesis is necessary \\
3.13 & Under the corresponding richness and local-deformability hypotheses, compatibility determines neither the joint nor the continuum section & \textsc{established} & Proved in text; zero-dimensional supports exhausted by the design are the explicit exception to the section claim \\
