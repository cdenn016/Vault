...
A second limitation belongs beside that one and is independent of it. Even granting a fixed point and a well-defined exponent at it, equality of $\gamma$ across two populations would not by itself place them in one universality class. An exponent is one number read off the low end of one spectral measure, and two flows agreeing on it may still differ in the limiting measure itself, in the amplitude multiplying the leading power, in the correction-to-scaling exponents governing the approach, in the rate at which the rescaling drives the self sector to zero, and in which invariant faces of the cone their orbits occupy, the Sylvester rank of Proposition~10.11 being one such face by \eqref{eq:rg-interior-ray}. A universality class is the full collection of quantities invariant under the flow's action on a basin; this chapter exhibits two candidate labels and does not show that they are complete.\status{OPEN} What would settle it is a classification of the flow-invariant functionals on a declared basin, which requires the basin first and therefore requires Open Problem~10.5.
...
Given it, one further step is usually taken without notice, and it is the step this section declines to take silently. Proposition~10.12 says that a component's parameters evolve on their own; it does not say that they evolve toward anything, nor that what they evolve toward is one thing, nor that what they evolve toward is a property of the population rather than of the coarse-graining choice. Three separate statements are needed before a component may be said to possess an effective law, and none of the three is proved in this document. The first is convergence, which is Open Problem~10.5 and which Conjecture~10.8 would settle on the coupling sector of a hierarchical family. The other two are recorded here, at the point where they are consumed, because they have not been named before.

\paragraph{Open Problem 10.13 (uniqueness of the limit within a component).}
Fix a connected population and an admissible blocking sequence for which the orbit of \eqref{eq:rg-flow} converges projectively. Is the limiting ray then independent of the initial parameters, either on the whole component parameter cone or on a declared basin inside it? Conjecture~10.8 would supply this on the coupling sub-cone of a hierarchical family, since a Birkhoff contraction has exactly one fixed ray and attracts all of the punctured cone; off that family nothing above excludes several limits, and Section~\ref{sec:rg-birkhoff} records explicitly that a failure of contraction leaves several fixed rays available inside one connected population. What would settle it is a uniqueness theorem on a declared class, or a pair of orbits in one component with distinct projective limits.\status{OPEN}

\paragraph{Open Problem 10.14 (independence of the blocking scheme).}
Fix a connected population whose orbits converge, and let two admissible blocking sequences be given, each with its declared rescaling. Do the two projective limits agree, and do the labels of Section~\ref{sec:rg-universality} take the same values on them? Nothing above constrains the answer: \eqref{eq:rg-flow} is defined relative to a declared sequence $S_\ell$ and the declaration is part of the scheme, not of the population. Section~\ref{sec:rg-gate} used consecutive pairing on an exchangeable draw and tried no alternative, so the present numerical evidence bears on one scheme only and is silent here. What would settle it is either an invariance theorem over a declared class of admissible blockings, or one population with two admissible blockings whose limits, or whose spectral exponents, differ.\status{OPEN}
...
Def.~10.10, spectral exponent & DEFINITION & A declared label; nothing is proved by declaring it, and it is not shown to be a complete one. \\
...
Equality of one spectral exponent does not identify a universality class & OPEN & Two flows agreeing on $\gamma$ may still differ in the limiting measure, the amplitude of the leading power, the correction-to-scaling exponents, the rate at which the self sector is driven to zero, and which invariant face of the cone the orbit occupies. Owed: a classification of the flow-invariant functionals on a declared basin, which needs Open~10.5 first. The chapter exhibits two candidate labels and does not show them complete. \\
...
Prop.~10.12, flow respects components & ESTABLISHED & Nothing, for blocking rules whose clusters do not straddle components. Each component carries its own orbit; the proposition asserts neither that an orbit converges nor that a component has one fixed ray rather than several. Flatness-free as a decomposition; the reading that coupled agents share a flow inherits Eq.~\eqref{eq:rg-trivializing}. \\
An observation is an edge & HYPOTHESIS & Interpretive commitment; everything in Sec.~\ref{sec:rg-disconnection} rests on it. \\
Open~10.13, uniqueness of the limit within a component & OPEN & A uniqueness theorem on a declared class, or a pair of orbits in one component with distinct projective limits. Conj.~10.8 would supply it on the coupling sub-cone of a hierarchical family and nowhere else. \\
Open~10.14, independence of the blocking scheme & OPEN & An invariance theorem over a declared class of admissible blockings, or one population whose two admissible blockings give different limits or different exponents. Sec.~\ref{sec:rg-gate} tried one scheme only and is silent here. \\
Cor.~10.15, the disconnection reading as an implication & ESTABLISHED & The implication only, and nothing more. Its antecedents are the edge hypothesis above together with Open~10.5, Open~10.13 and Open~10.14, none of which is settled. \\
No observable variation in the effective law, unconditionally & NOT-CLAIMED & Declined, not refuted, and asserted nowhere in this document without the antecedents of Cor.~10.15. Prop.~10.12 alone gives one dynamically independent subsystem per component, not one reached limit. Sec.~\ref{sec:rg-gate} bears on convergence only and not on uniqueness or scheme independence. Even granted, it is a constraint the theory places on itself and not a test it can pass. \\
