...
\paragraph{Canonical-correlation form and stable realization.}
For a two-block precision, write
\begin{equation}
...
\begin{equation}
D-C^\top A^{-1}C
=L_D\left(I-R^\top R\right)L_D^\top,
\qquad
\det J=\det A\,\det D\,\det\left(I-R^\top R\right).
...
\begin{equation}
\Delta(A,D;C)
=-\frac12\log\det\left(I-R^\top R\right)
=-\frac12\sum_a\log\left(1-\rho_a^2\right).
\label{eq:restrict-canonical-correlation-gap}
\end{equation}
For more than two blocks, successively merging the first block with the next
block in the declared partition order telescopes these increments to
Equation~\eqref{eq:restrict-determinant-gap}; no ordering claim beyond this
exact telescoping identity is needed.
...
\texttt{-0.5*log1p(-rho**2)}.  It neither forms $A^{-1}$ or $D^{-1}$ nor
subtracts large floating-point log determinants.  A separate boundary policy
handles a squared singular value within an operational $64n\epsilon_{\rm mach}$
neighborhood of one.  There the exact stored binary64 entries are reevaluated
at 200 decimal digits, including the determinant-gap value; a second
binary64 solve backend makes the diagnostic the maximum excursion observed
across the two declared binary64 backends.
...
a universal error threshold.  An ill-conditioned but in-domain value is
reevaluated rather than rejected merely for triggering that path.  These
