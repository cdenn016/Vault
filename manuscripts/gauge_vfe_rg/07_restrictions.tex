...
path.  When a pure high-precision fallback bypasses the ordinary binary64
factorization, its Cholesky residual, solve residual, residual tolerance, and
backward-error diagnostics are recorded as \texttt{null}: they were not
evaluated, and \texttt{null} denotes neither failure nor zero.  The zero-step
one-block case retains its exact zero diagnostics.  These guarded fallbacks are
declared finite-precision procedures, not theorems that
