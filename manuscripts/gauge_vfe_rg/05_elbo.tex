...
\end{equation}
so that \(Q_X(\mathsf Y_D\given o)=1\) and, for every \(B\in\mathscr Y_D\), the map \((o,X)\mapsto Q_X(B\given o)\) is \(\mathscr O_D\otimes\mathscr X\)-measurable. \status{DEFINITION}
...
nondegenerate \(Q_X\) the expected residual second moments are positive
...
for arbitrary $Q_X$ whenever each candidate has a selected positive-finite
...
singular-branch sum $\Lelbo^{\mathrm{ext}}+\KL$. Under (H3)--(H4) at both
...
$Q_X=P_{\theta^{\mathrm{old}}}(\cdot\given o,X)$, before each accepted M phase.
