...
these laws form only a sequence of coarse steps. \status{ESTABLISHED} If local comparison functions
\(a^s_\ell\) satisfy
\[
...
\(T_\ell:D_\ell\to X_{\ell+1}\) be continuously Fr\'echet differentiable on an
open set \(D_\ell\subseteq X_\ell\). The ordered nonlinear evolution is
\begin{equation}
...
\begin{equation}
M_\ell=DT_\ell(x_\ell)\in\mathcal L(X_\ell,X_{\ell+1}),
\qquad
M_{n\leftarrow\ell}=M_{n-1}\cdots M_\ell,
\qquad
M_{\ell\leftarrow\ell}=I_{X_\ell}.
...
\qquad
M_{n\leftarrow\ell}=D\Phi_{n\leftarrow\ell}(x_\ell),
\qquad
M_{r\leftarrow n}M_{n\leftarrow\ell}=M_{r\leftarrow\ell}.
...
\(D\Phi_{n+1\leftarrow\ell}(x_\ell)=DT_n(x_n)M_{n\leftarrow\ell}
=M_nM_{n\leftarrow\ell}\). The third identity is the associativity of the
...
\begin{equation}
M_\ell v_{\ell,a}=\lambda_{\ell,a}v_{\ell+1,a}.
\label{eq:rg-mode-line}
\end{equation}
This is a compatibility equation between adjacent fibers. The expression
\(M_\ell v=\lambda v\) is not used: its two sides inhabit \(X_{\ell+1}\) and
...
\begin{equation}
M_{n\leftarrow\ell}v_{\ell,a}=\lambda_{n\leftarrow\ell,a}v_{n,a},
\qquad
...
\(M_{n+1\leftarrow\ell}v_{\ell,a}=M_nM_{n\leftarrow\ell}v_{\ell,a}
=\lambda_{n\leftarrow\ell,a}M_nv_{n,a}
...
\(X_0=\R\), \(X_1=\R^2\), and \(M_0x=(x,0)\), the expression \(M_0v=\lambda v\)
...
\qquad
\widehat M_{n\leftarrow\ell}=J_n^{-1}M_{n\leftarrow\ell}J_\ell.
...
computed for \(\widehat M_{n\leftarrow\ell}\) in the reference norm. Together
...
\paragraph{Proof.}
Write \(x_n=M_{n\leftarrow\ell}u\) and
\(\widehat x_n=J_n^{-1}x_n=\widehat M_{n\leftarrow\ell}J_\ell^{-1}u\), which is
...
\(X_k=\R\) with the native absolute value, let every \(M_k\) be the identity, let
...
\(J_ku=e^{k^2}u\). Then \(\widehat M_k=e^{-2k-1}I\) and
\(\widehat M_{n\leftarrow0}=e^{-n^2}I\), so the apparent reference-space rate is
...
\paragraph{Proof.}
\(\widehat M_k=J_{k+1}^{-1}J_k=e^{k^2-(k+1)^2}=e^{-2k-1}\), and the telescoping
...
cocycle in the base variable, integrability of \(\log^+\lVert M\rVert\) and, for
the two-sided splitting, of \(\log^+\lVert M^{-1}\rVert\), and either finite
