...
\propositionheading{Constructive nonemptiness}{prop:gauss-interaction-nonempty}
Under \Cref{hyp:gauss-flat-comparison}, let each edge of a graph on $V$ carry an agreement factor with residual $\mu_i-\Theta_{ij}\mu_j$ and link covariance $R_{ij}\in\Sym_{++}^{K}$, and let each agent carry a proper prior of precision $A_i\succeq0$ in the trivialized coordinates. Then the assembled trivialized energy is
\begin{equation}
