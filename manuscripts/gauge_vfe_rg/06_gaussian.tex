...
\propositionheading{Log normalizer, moments, and convexity}{prop:gauss-log-normalizer-moments}
...
\paragraph{Precision blocks are not covariance values.}
...
The distinction exhibited by \eqref{eq:gauss-precision-not-covariance} is
the reason this document works in information form. A restriction imposed
on the recognition family is a restriction on conditional dependence, and
it is stated cleanly only in coordinates in which conditional dependence is
a coordinate. The exact price is computed in \Cref{ch:restrictions}.
...
The state latents carry frames. A residual between two agents is not $k_i-k_j$, which compares vectors in different fibers, but $k_i-\Theta_{ij}k_j$, where $\Theta_{ij}$ is the represented belief-channel graph link. The interaction shape of \Cref{def:gauss-interaction-family} involves plain differences, so it can only be a statement in coordinates in which the graph links have been removed. When that removal is possible is settled exactly by the following.

\propositionheading{Simultaneous trivialization is exactly coboundary}{prop:gauss-coboundary-trivialization}
Let $G=(V,E)$ be a connected graph and let $\{\Theta_{ij}\}_{(i,j)\in E}\subset\GL^{+}(K)$ satisfy $\Theta_{ji}=\Theta_{ij}^{-1}$. There exist frames $\{U_i\}\subset\GL^{+}(K)$ with $U_i^{-1}\Theta_{ij}U_j=I$ for every edge if and only if $\Theta_{ij}=U_iU_j^{-1}$ for those frames, and this holds if and only if the ordered product of graph links around every cycle of $G$ is the identity. When it holds, the frames are unique up to a common right factor $U_i\mapsto U_ig$ with $g\in\GL^{+}(K)$ independent of $i$. \status{ESTABLISHED}

\paragraph{Proof.}
The condition $U_i^{-1}\Theta_{ij}U_j=I$ is literally $\Theta_{ij}=U_iU_j^{-1}$, which gives the first equivalence. For the second, suppose such frames exist and let $i_0,i_1,\dots,i_r=i_0$ be a cycle. Then $\prod_{t}\Theta_{i_ti_{t+1}}=\prod_t U_{i_t}U_{i_{t+1}}^{-1}=U_{i_0}U_{i_0}^{-1}=I$ by telescoping. Conversely, assume every cycle product is the identity, fix a spanning tree and a root $r$, set $U_r=I$, and define $U_i$ as the ordered product of graph links along the tree path from $r$ to $i$. The relation $\Theta_{ij}=U_iU_j^{-1}$ then holds on every tree edge by construction, and on a non-tree edge it holds precisely because the cycle formed by that edge and the tree path between its endpoints has trivial product. For uniqueness, if $U_iU_j^{-1}=U_i'U_j'^{-1}$ on every edge then $U_i'^{-1}U_i=U_j'^{-1}U_j$ for adjacent $i,j$, so this quantity is constant on a connected graph. $\square$

\hypothesisheading{Flat comparison}{hyp:gauss-flat-comparison}
The declared graph links are a coboundary, $\Theta_{ij}=U_iU_j^{-1}$, so that the trivialization $z_i=U_i^{-1}\mu_i$ of the means, and the same map $z_i=U_i^{-1}k_i$ applied to the latents themselves, turn every residual into a plain difference,
\begin{equation}
\mu_i-\Theta_{ij}\mu_j=U_i\left(z_i-z_j\right).
\label{eq:gauss-flat-trivialization}
\end{equation}
This is a choice, not a derivation. By \Cref{prop:gauss-coboundary-trivialization} it excludes exactly the graph-link assignments with nontrivial holonomy around some cycle, which are permitted in Regime II. It is used in two places: it makes the interaction shape of \Cref{def:gauss-interaction-family} a statement in one fixed coordinate system, and it supplies a clean sufficient domain for closure under aggregation. In the flat chart the represented link
\(\widehat\Theta_{ij}:=U_i^{-1}\Theta_{ij}U_j\) equals \(I\), so merging two
agents by \(z_i=z_j=z\) annihilates their internal residual. More generally,
in an arbitrary common chart the internal edge energy becomes
\[
z^\top(I-\widehat\Theta_{ij})^\top
W_{ij}(I-\widehat\Theta_{ij})z,
\]
which vanishes for every \(z\) exactly when
\(W_{ij}^{1/2}(I-\widehat\Theta_{ij})=0\). Positive-definite \(W_{ij}\), or
faithfulness of the weight on every represented direction, reduces this
condition to \(\widehat\Theta_{ij}=I\); with merely positive-semidefinite
weights, a nontrivial represented link can survive in invisible null
directions. Thus \Cref{prop:gauss-coboundary-trivialization} remains the exact
coboundary theorem, while the converse from energy annihilation to a trivial
represented link requires the stated nondegeneracy. The coarse-graining
chapter carries out the weighted computation and separates the sufficient
trivializing domain from degenerate exceptions. \status{HYPOTHESIS}
...
\begin{equation}
k_i\given k_j\sim\mathcal N\left(G_i\Theta_{ij}k_j,\Lambda_i^{-1}\right),
\qquad
\Lambda_i\in\Sym_{++}^{K},
\qquad
G_i\in\R^{K\times K},
...
\end{equation}
and each root $r$ carries a proper prior $\mathcal N(0,\Lambda_r^{-1})$, a nonzero prior mean shifting only the information vector $h$ and not the precision. The gain is written $G_i$ rather than $A_i$ because the latter symbol is reserved for the self terms of \Cref{def:gauss-interaction-family}. Unlike the represented graph link \(\Theta_{ij}\), the conditional mean map \(G_i\) need not be invertible: positive definiteness of the innovation covariance normalizes the receiving kernel for every matrix \(G_i\), exactly as \Cref{ch:generative} declares. A single subscript on $\Lambda$ denotes the innovation precision of one receiving kernel; a double subscript denotes a block of the assembled precision. When an invertible-gain submodel is wanted it will be named explicitly.

\propositionheading{Assembled precision blocks}{prop:gauss-assembled-precision-blocks}
Write $T_i:=G_i\Theta_{i\operatorname{pa}(i)}$ and $\operatorname{ch}(i)=\{\ell:\operatorname{pa}(\ell)=i\}$. The joint precision of the model above has blocks
\begin{equation}
...
\begin{equation}
W_{i\operatorname{pa}(i)}:=\Lambda_iT_i=\Lambda_iG_i\Theta_{i\operatorname{pa}(i)}
...
Two sources of failure must be kept apart, and a convention does it. Under \Cref{hyp:gauss-flat-comparison} the trivialized residual is $z_i-\widetilde T_iz_j$ with
\begin{equation}
...
\end{equation}
the second equality in the first display using $\Theta_{ij}=U_iU_j^{-1}$, so that \eqref{eq:gauss-directed-blocks} holds verbatim in trivialized coordinates with $(\Lambda_i,T_i)$ replaced by $(\widetilde\Lambda_i,\widetilde T_i)$. The graph links have then already been removed and any remaining obstruction is attributable to the gain alone. For the rest of this section the tildes are dropped and $(\Lambda_i,T_i)$ denote the trivialized quantities.

\propositionheading{Two global-condition witnesses}{prop:gauss-global-condition-witnesses}
...
\end{equation}
which differs from its own transpose by a matrix of Frobenius norm $5\sqrt2$, so the off-diagonal block is not symmetric and no $W$ exists at all. Separately, taking $K=1$ with $G_i=-1$ and $\Theta_{ij}=\Lambda_i=1$ gives a symmetric block with $W=-1<0$, so the sign requirement fails; the same construction with $G_i=-I$, $\Theta_{ij}=I$ and $\Lambda_i=I$ gives $W=-I\prec0$ at every $K$. \status{ESTABLISHED}

\paragraph{Proof.}
Direct computation. For the first, $\Lambda_iT_i=\begin{psmallmatrix}-1&2\\-3&1\end{psmallmatrix}$, so $-\Lambda_iT_i$ is as displayed and $-\Lambda_iT_i-(-\Lambda_iT_i)^\top=\begin{psmallmatrix}0&-5\\5&0\end{psmallmatrix}$ with Frobenius norm $\sqrt{50}$. Positive definiteness of $\Lambda_i$ follows from $\det=5>0$ and trace $5>0$; invertibility of $T_i$ from $\det T_i=1>0$. For the second, \eqref{eq:gauss-candidate-weight} evaluates to $\Lambda_iG_i\Theta_{ij}=-I$, and $-I$ is not positive semidefinite for any $K\geq1$. $\square$

\Cref{prop:gauss-global-condition-witnesses} settles the status of the interaction shape. The gain matrix $G_i$ is a free generative parameter in the construction of the preceding part, not required to be symmetric, positive, or even close to the identity, and for the displayed values the assembled precision has no representation of the form of Equation~\eqref{eq:gauss-interaction-family}. Whatever else may be said for the interaction family, it is not a consequence of the general linear-Gaussian directed model.
...
If the receiving gain \(T_i\) is required to be invertible, the isolated factor lies in \(\mathcal I(\{i,j\})\) if and only if \(T_i=I\). Indeed, \Cref{prop:gauss-edge-local-characterization} makes \(T_i\) idempotent, and multiplying \(T_i^2=T_i\) by \(T_i^{-1}\) gives \(T_i=I\); the converse is the full-rank case of that proposition. \status{ESTABLISHED}

\Cref{prop:gauss-edge-local-characterization} describes the stronger factor-local requirement, not global family membership. With singular gains allowed, a proper projection constrains only the components of the residual inside \(\operatorname{range}(X)\) and leaves the complement to the receiving-side self term. In the invertible-gain submodel, \Cref{cor:gauss-invertible-gain-factor} removes every proper projection and leaves only pure agreement. The deterministic replacement check \texttt{CHK-GAUSS-PROJECTION} tests $4000$ generic gains and exact controls of ranks $0$ through $3$ under a fully specified protocol; its current output corroborates the characterization but does not prove generic measure zero. \status{NUMERICAL}

\hypothesisheading{Global interaction form}{hyp:gauss-global-interaction}
The renormalization half of this document acts on recognition precisions that belong globally to Equation~\eqref{eq:gauss-interaction-family}, in trivialized coordinates, and this is adopted as a declared restriction. For a precision assembled from the directed model of Equation~\eqref{eq:gauss-receiving-kernel}, the restriction is exactly \Cref{prop:gauss-global-interaction-condition}; it does \emph{not} require every receiving factor to satisfy \Cref{prop:gauss-edge-local-characterization}, because the balance terms in Equation~\eqref{eq:gauss-global-cond-2} may cancel as Equation~\eqref{eq:gauss-global-cancellation-witness} proves. It is used in the aggregation closure statement, in the definition of the flow, and in the fixed-point analysis; each of those results names it. The document does not claim that it is forced by anything, and \Cref{prop:gauss-interaction-family-thin} records that it is a measure-zero restriction on the space of Gaussian recognition laws. \status{HYPOTHESIS}

\hypothesisheading{Optional factorwise-agreement realization}{hyp:gauss-factorwise-agreement}
When the interaction parameters are required to be readable from the generative model one factor at a time, every receiving factor is separately assumed to satisfy \Cref{prop:gauss-edge-local-characterization}. This is a strictly stronger hypothesis than global membership: it permits \(\Lambda_i\)-orthogonal projections in the relaxed singular-gain model and only \(T_i=I\) in the invertible-gain model, while it excludes the globally admissible witness \eqref{eq:gauss-global-cancellation-witness}. The operator-level renormalization results require \Cref{hyp:gauss-global-interaction}, not this stronger factorwise realization, unless they explicitly invoke a factor interpretation. \status{HYPOTHESIS}
...
\propositionheading{Constructive nonemptiness and exact kernel}{prop:gauss-interaction-nonempty}
Under \Cref{hyp:gauss-flat-comparison}, let each edge of a graph
$G=(V,E)$ carry an agreement factor with residual
$\mu_i-\Theta_{ij}\mu_j$ and link covariance
$R_{ij}\in\Sym_{++}^{K}$, and let each vertex carry a quadratic self
term $A_i\succeq0$ in the trivialized coordinates. A self term is the
precision of a proper full-dimensional Gaussian prior at vertex $i$ only
when $A_i\succ0$. The assembled trivialized energy is
\begin{equation}
...
\end{equation}
where the edge sum is over $E$. The associated precision
$\Lambda$ lies in $\mathcal I(V)\cap\PSD^{NK}$, every represented edge
weight is positive definite, and
\begin{equation}
\ker\Lambda
...
\end{equation}
If $V_\alpha$ are the connected components of $G$, then equivalently
\begin{equation}
\ker\Lambda
=\bigoplus_\alpha
...
$\bigcap_{i\in V_\alpha}\ker A_i=\{0\}$ for every connected component.
In particular, one positive-definite self term in each component suffices;
taking every $A_i\succ0$ makes every vertex self term a proper prior and
constructs an element of
$\mathcal I(V)\cap\Sym_{++}^{NK}$. \status{ESTABLISHED}

\paragraph{Proof.}
By \eqref{eq:gauss-flat-trivialization} the residual is $U_i(z_i-z_j)$, so the quadratic form $(\mu_i-\Theta_{ij}\mu_j)^\top R_{ij}^{-1}(\mu_i-\Theta_{ij}\mu_j)$ equals $(z_i-z_j)^\top U_i^\top R_{ij}^{-1}U_i(z_i-z_j)$. The matrix $U_i^\top R_{ij}^{-1}U_i$ is symmetric and positive definite because $R_{ij}^{-1}$ is and $U_i$ is invertible. Comparing with \eqref{eq:gauss-interaction-energy} identifies the parameters and proves positive semidefiniteness. Equality in the energy holds exactly when $A_iz_i=0$ at every vertex and, because every $W_{ij}\succ0$, when $z_i=z_j$ on every edge. The latter conditions make $z$ constant on each connected component, which proves \eqref{eq:gauss-nonempty-kernel} and \eqref{eq:gauss-nonempty-component-kernel}. A positive-semidefinite matrix is positive definite exactly when its kernel is trivial, yielding the final criterion and the stated proper witnesses. $\square$

\Cref{prop:gauss-interaction-nonempty} also makes the role of \Cref{hyp:gauss-flat-comparison} concrete by contrast. Without trivialization the same agreement factor contributes the off-diagonal block $-R_{ij}^{-1}\Theta_{ij}$, which is symmetric only in special position. The deterministic replacement check \texttt{CHK-GAUSS-TRIVIALIZATION} emits its matrices and compares that asymmetric raw block with the symmetric positive-definite trivialized weight. It corroborates the proposition and is not part of its proof. \status{NUMERICAL}

Conditioning is a different question and admits only protocol-specific computational evidence. The deterministic replacement check \texttt{CHK-GAUSS-CONDITIONING} specifies and runs a $200$-draw interaction ensemble together with a zero-self consensus control and $200$ unstructured positive-definite controls. Its output concerns only that sampler. Positive semidefiniteness and the general null-space criterion are proved by \Cref{prop:gauss-interaction-energy-kernel}; \Cref{prop:gauss-interaction-nonempty} specializes that criterion to the constructed positive-definite edge weights and supplies explicit proper witnesses. Thinness is proved by \Cref{prop:gauss-interaction-family-thin}; no universal conditioning theorem is claimed. \status{NUMERICAL}

\section{Marginalization belongs to the Gaussian coarse chapter}
\label{sec:gauss-open}

Aggregation identifies variables, whereas exact Gaussian marginalization
eliminates them by a Schur complement. The interaction cone is closed under
the former and not under unrestricted matrix-weighted Kron reduction. The
exact positive-definite \(N=3,K=2\) rational counterexample, including the
asymmetric manufactured coupling, is stated and proved once in
\Cref{ch:gaussian-coarsegraining}. \status{ESTABLISHED}

The same chapter gives the positive result in its sharp current form: every
fixed congruence-diagonal cone
\(\mathcal C_H=\{HDH^\top:D\succeq0\text{ diagonal}\}\) is closed under
every defined node-block Schur elimination. This strictly contains the
common-orthogonal-eigenbasis case because congruence-diagonal matrices need
not commute. Maximality is proved only for rich, independently parameterized
closed convex product cones with an SPD order unit and closure under every
defined one-node elimination. Correlated, nonconvex, variable-chart,
rank-changing, and nonflat families remain outside that theorem.
\status{ESTABLISHED}
