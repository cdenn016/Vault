...
\paragraph{Proposition 7.1 (log normalizer, moments, convexity).}
...
\paragraph{Proposition 7.2 (conditional reading of the precision).}
...
\paragraph{Remark 7.3 (a precision block is not a covariance value).}
...
The distinction in Remark 7.3 is the reason this document works in information form. A restriction imposed on the recognition family is a restriction on conditional dependence, and it is stated cleanly only in the coordinates in which conditional dependence is a coordinate. The exact price of such a restriction is computed in the next chapter.

\section{The declared interaction subfamily}
\label{sec:gauss-interaction-family}

\paragraph{Definition 7.4 (interaction family).}
...
\paragraph{Proposition 7.5 (energy, semidefiniteness, kernel).}
...
Proposition 7.6 is the quantitative form of the claim that the interaction shape is a restriction in a fixed product chart rather than a normalization. It is not invariant under arbitrary independent block-diagonal coordinate changes, so membership may be created or destroyed by such a change and no coordinate-orbit exclusion follows from thinness. An exact witness starts from the interaction precision with \(N=2\), \(K=2\), \(A_1=A_2=I_2\), and \(W_{12}=I_2\). Congruence by \(G=\operatorname{diag}(I_2,S)\), \(S=\left(\begin{smallmatrix}1&1\\0&1\end{smallmatrix}\right)\), changes the off-diagonal block from \(-I_2\) to \(-S\), which is not symmetric and hence is outside the family; applying \(G^{-1}\) to that transformed precision creates membership again. The invariant statement is only that a declared trivialization must be fixed before Equation~\eqref{eq:gauss-interaction-family} is imposed. Section~\ref{sec:gauss-hypothesis} asks when the generative construction supplies such a distinguished chart.
...
The state latents carry frames. A residual between two agents is not $k_i-k_j$, which compares vectors in different fibers, but $k_i-\Omega_{ij}k_j$, where $\Omega_{ij}$ is the declared transport along the edge. The interaction shape of Definition 7.4 involves plain differences, so it can only be a statement in coordinates in which the transports have been removed. When that removal is possible is settled exactly by the following.

\paragraph{Proposition 7.7 (simultaneous trivialization is exactly coboundary).}
...
\paragraph{Hypothesis 7.8 (flat comparison).}
...
\end{equation}
This is a choice, not a derivation. By Proposition 7.7 it excludes exactly the link assignments with nontrivial holonomy around some cycle, which are permitted in Regime II. It is used in two places: it makes the interaction shape of Definition 7.4 a statement in one fixed coordinate system, and it supplies a clean sufficient domain for closure under aggregation. The mechanism of the second use is visible already: merging two agents identifies $z_i=z_j=z$, and the internal edge energy $(z_i-\widetilde\Omega_{ij}z_j)^\top W_{ij}(z_i-\widetilde\Omega_{ij}z_j)$ collapses to $z^\top(I-\widetilde\Omega_{ij})^\top W_{ij}(I-\widetilde\Omega_{ij})z$. This residue vanishes for every $z$ exactly when \(W_{ij}^{1/2}(I-\widetilde\Omega_{ij})=0\). Positive-definite \(W_{ij}\), or faithfulness of the weight on every represented direction, reduces that condition to \(\widetilde\Omega_{ij}=I\); with merely positive-semidefinite weights, nontrivial transport can survive in invisible null directions. Proposition 7.7 remains the exact coboundary theorem, while equivalence between coboundary transport and energy annihilation requires the stated nondegeneracy. The coarse-graining chapter carries out the weighted computation and separates the sufficient trivializing domain from degenerate exceptions. \status{HYPOTHESIS}
...
\end{equation}
and each root $r$ carries a proper prior $\mathcal N(0,\Lambda_r^{-1})$, a nonzero prior mean shifting only the information vector $h$ and not the precision. The gain is written $G_i$ rather than $A_i$ because the latter symbol is reserved for the self terms of Definition 7.4. Unlike the represented transport \(\Omega_{ij}\), the conditional mean map \(G_i\) need not be invertible: positive definiteness of the innovation covariance normalizes the receiving kernel for every matrix \(G_i\), exactly as Chapter~\ref{ch:generative} declares. A single subscript on $\Lambda$ denotes the innovation precision of one receiving kernel; a double subscript denotes a block of the assembled precision. When an invertible-gain submodel is wanted it will be named explicitly.

\paragraph{Proposition 7.9 (assembled precision blocks).}
...
Two sources of failure must be kept apart, and a convention does it. Under Hypothesis 7.8 the trivialized residual is $z_i-\widetilde T_iz_j$ with
\begin{equation}
...
\paragraph{Proposition 7.10 (two witnesses).}
...
Proposition 7.10 settles the status of the interaction shape. The gain matrix $G_i$ is a free generative parameter in the construction of the preceding part, not required to be symmetric, positive, or even close to the identity, and for the displayed values the assembled precision has no representation of the form of Equation~\eqref{eq:gauss-interaction-family}. Whatever else may be said for the interaction family, it is not a consequence of the general linear-Gaussian directed model.
...
\paragraph{Proposition 7.11 (exact global condition).}
...
The cancellation is not merely formal. Take one scalar receiving factor \(j\to i\) with
\(\Lambda_i=1\), \(T_i=\tfrac12\), and root precision \(\Lambda_j=1\). In the block order \((i,j)\), Equation~\eqref{eq:gauss-directed-blocks} gives
\begin{equation}
...
\end{equation}
Thus the assembled precision belongs to \(\mathcal I(\{i,j\})\), although
\(T_i^2=\tfrac14\neq\tfrac12=T_i\), so the receiving factor does not belong locally: its parent-side residual is \(T_i^2\Lambda_i-W=\tfrac14-\tfrac12=-\tfrac14\), and the root prior offsets it to \(A_j=1-\tfrac14=\tfrac34\). The matrix is positive definite because its leading principal minors are \(1\) and \(1\). This exact witness proves that factorwise membership is sufficient but not necessary for global membership. \status{ESTABLISHED}

\paragraph{Proposition 7.12 (edge-local characterization with singular gains allowed).}
...
\paragraph{Corollary 7.12a (invertible-gain factor).}
If the receiving gain \(T_i\) is required to be invertible, the isolated factor lies in \(\mathcal I(\{i,j\})\) if and only if \(T_i=I\). Indeed, Proposition~7.12 makes \(T_i\) idempotent, and multiplying \(T_i^2=T_i\) by \(T_i^{-1}\) gives \(T_i=I\); the converse is the full-rank case of that proposition. \status{ESTABLISHED}

Proposition 7.12 describes the stronger factor-local requirement, not global family membership. With singular gains allowed, a proper projection constrains only the components of the residual inside \(\operatorname{range}(X)\) and leaves the complement to the receiving-side self term. In the invertible-gain submodel, Corollary~7.12a removes every proper projection and leaves only pure agreement. The deterministic replacement check \texttt{CHK-GAUSS-PROJECTION} tests $4000$ generic gains and exact controls of ranks $0$ through $3$ under a fully specified protocol; its current output corroborates the characterization but does not prove generic measure zero. \status{NUMERICAL}

\paragraph{Hypothesis 7.13 (global interaction form).}
The renormalization half of this document acts on recognition precisions that belong globally to Equation~\eqref{eq:gauss-interaction-family}, in trivialized coordinates, and this is adopted as a declared restriction. For a precision assembled from the directed model of Equation~\eqref{eq:gauss-receiving-kernel}, the restriction is exactly Proposition~7.11; it does \emph{not} require every receiving factor to satisfy Proposition~7.12, because the balance terms in Equation~\eqref{eq:gauss-global-cond-2} may cancel as Equation~\eqref{eq:gauss-global-cancellation-witness} proves. It is used in the aggregation closure statement, in the definition of the flow, and in the fixed-point analysis; each of those results names it. The document does not claim that it is forced by anything, and Proposition 7.6 records that it is a measure-zero restriction on the space of Gaussian recognition laws. \status{HYPOTHESIS}

\paragraph{Hypothesis 7.13a (optional factorwise-agreement realization).}
When the interaction parameters are required to be readable from the generative model one factor at a time, every receiving factor is separately assumed to satisfy Proposition~7.12. This is a strictly stronger hypothesis than global membership: it permits \(\Lambda_i\)-orthogonal projections in the relaxed singular-gain model and only \(T_i=I\) in the invertible-gain model, while it excludes the globally admissible witness \eqref{eq:gauss-global-cancellation-witness}. The operator-level renormalization results require Hypothesis~7.13, not this stronger factorwise realization, unless they explicitly invoke a factor interpretation. \status{HYPOTHESIS}
...
\paragraph{Proposition 7.14 (constructive nonemptiness).}
Under Hypothesis 7.8, let each edge of a graph on $V$ carry an agreement factor with residual $\mu_i-\Omega_{ij}\mu_j$ and link covariance $R_{ij}\in\Sym_{++}^{K}$, and let each agent carry a proper prior of precision $A_i\succeq0$ in the trivialized coordinates. Then the assembled trivialized energy is
\begin{equation}
...
Proposition 7.14 also makes the role of Hypothesis 7.8 concrete by contrast. Without trivialization the same agreement factor contributes the off-diagonal block $-R_{ij}^{-1}\Omega_{ij}$, which is symmetric only in special position. The deterministic replacement check \texttt{CHK-GAUSS-TRIVIALIZATION} emits its matrices and compares that asymmetric raw block with the symmetric positive-definite trivialized weight. It corroborates the proposition and is not part of its proof. \status{NUMERICAL}

Conditioning is a different question and admits only protocol-specific computational evidence. The deterministic replacement check \texttt{CHK-GAUSS-CONDITIONING} specifies and runs a $200$-draw interaction ensemble together with a zero-self consensus control and $200$ unstructured positive-definite controls. Its output concerns only that sampler. Positive definiteness is proved by Proposition 7.5 and thinness by Proposition 7.6; no universal conditioning theorem is claimed. \status{NUMERICAL}

\section{Marginalization: a negative theorem, a positive subfamily, and one classification problem}
\label{sec:gauss-open}

\paragraph{Proposition 7.15 (unrestricted matrix-weighted Kron closure fails).}
Aggregation identifies agents; exact marginalization eliminates them and replaces \(\Lambda\) by a Schur complement. The interaction family is not closed under the latter operation for \(K\geq2\). An exact witness has \(N=3\), \(K=2\),
\begin{equation}
A_1=A_2=A_3=I_2,\qquad
W_{12}=0,\qquad
W_{13}=\begin{pmatrix}1&0\\0&2\end{pmatrix},\qquad
W_{23}=\begin{pmatrix}2&1\\1&2\end{pmatrix}.
\label{eq:gauss-kron-witness}
\end{equation}
All self terms and weights are positive semidefinite. The assembled precision has leading principal minors
\(2,6,18,48,102,233\), hence is positive definite. Eliminating agent \(3\), whose diagonal block is
\(\Lambda_{33}=\begin{psmallmatrix}4&1\\1&5\end{psmallmatrix}\), creates the retained coupling
\begin{equation}
-\bigl(\Lambda/\Lambda_{33}\bigr)_{12}
=W_{13}\Lambda_{33}^{-1}W_{23}
=\frac1{19}\begin{pmatrix}9&3\\4&14\end{pmatrix}.
\label{eq:gauss-kron-asymmetric}
\end{equation}
This block is not symmetric, so it cannot be a weight in \(\PSD^2\), even though the full \(4\times4\) Schur complement is symmetric. Therefore the reduced precision is outside \(\mathcal I(\{1,2\})\). \status{ESTABLISHED}

\paragraph{Proof.}
For one eliminated agent \(e\), the new off-diagonal block between retained \(i\) and \(j\) is
\(-W_{ij}-W_{ie}\Lambda_{ee}^{-1}W_{ej}\). Membership requires the manufactured weight to be symmetric, equivalently
\(W_{ie}\Lambda_{ee}^{-1}W_{ej}=W_{ej}\Lambda_{ee}^{-1}W_{ie}\). In \eqref{eq:gauss-kron-witness},
\(\Lambda_{33}^{-1}=\tfrac1{19}\begin{psmallmatrix}5&-1\\-1&4\end{psmallmatrix}\), and direct multiplication gives \eqref{eq:gauss-kron-asymmetric}, whose two off-diagonal entries are \(3/19\) and \(4/19\). Positive definiteness of the fine precision follows from Sylvester's criterion and the displayed exact minors. This is the same witness used in \Cref{ch:coarsegraining}, where the distinction between aggregation and marginalization is developed. \(\square\)

\paragraph{Proposition 7.16 (a common-eigenbasis subfamily is Kron closed).}
Suppose one orthogonal matrix \(O\) simultaneously diagonalizes every self term \(A_i\) and every edge weight \(W_{ij}\). For every retained set \(R\) whose eliminated principal block \(\Lambda_{EE}\) is invertible, the Schur complement
\(\Lambda_{RR}-\Lambda_{RE}\Lambda_{EE}^{-1}\Lambda_{ER}\) again belongs to the interaction family, and all of its self terms and edge weights are diagonalized by \(O\). \status{ESTABLISHED}

\paragraph{Proof.}
Conjugate every fiber by \(O\) and permute coordinates so that equal eigenchannels are adjacent. The full precision becomes a direct sum of \(K\) scalar loopy Laplacians \(Q^{(a)}\), one per channel. Schur complementation respects direct sums, so it acts channel by channel. For one channel, write \(Q\mathbf1=a\geq0\) for its self-term vector and partition \(Q\) into retained and eliminated blocks. The invertible principal block \(Q_{EE}\) is a nonsingular symmetric \(M\)-matrix, hence \(Q_{EE}^{-1}\geq0\) entrywise. Therefore the reduced off-diagonal entries remain nonpositive, and its row sums are
\[
\bigl(Q_{RR}-Q_{RE}Q_{EE}^{-1}Q_{ER}\bigr)\mathbf1_R
=a_R-Q_{RE}Q_{EE}^{-1}a_E\geq0,
\]
because \(Q_{RE}\leq0\). Each reduced channel is another scalar loopy Laplacian. Reassembling the channels and conjugating by \(O\) gives positive semidefinite matrix weights and self terms in one common eigenbasis. The hypothesis must include the self terms, because \(\Lambda_{EE}^{-1}\), not only the edge weights, enters the manufactured coupling. \(\square\)

\paragraph{Open problem 7.17 (classification of Kron-closed subfamilies).}
The unrestricted closure question is closed negatively by Proposition~7.15, and the common-orthogonal-eigenbasis family is closed positively by Proposition~7.16. What remains open is the classification of maximal parameter subfamilies stable under every defined elimination. Passing the one-step symmetry condition
\(W_{ie}\Lambda_{ee}^{-1}W_{ej}=W_{ej}\Lambda_{ee}^{-1}W_{ie}\) is not sufficient by itself: the reduced weights must retain the positive-semidefinite sign, the defining parameter restrictions must survive so that later eliminations remain covered, and each ordinary Schur complement requires an invertible eliminated block. A solution must prove all three properties under iteration or exhibit a larger counterexample. \status{OPEN}
...
\bottomrule
\endlastfoot
Prop.~7.1 & Log normalizer, moments, convexity of the Gaussian information form & ESTABLISHED & Nothing; proved, with the regularity condition cited \\
Prop.~7.2 & Conditional mean and covariance; zero precision block equals conditional independence & ESTABLISHED & Nothing; proved and cited \\
Rem.~7.3 & A precision block is not a covariance value; marginal block is an inverse Schur complement & ESTABLISHED & Nothing; explicit witness given \\
Def.~7.4 & The interaction family $\mathcal I(V)$ & DEFINITION & Nothing is proved by a definition \\
Prop.~7.5 & Energy form, semidefiniteness, exact definiteness criterion, consensus kernel & ESTABLISHED & Nothing; proved \\
Prop.~7.6 & The family is Lebesgue-null in $\Sym^{NK}$ for $K\geq2$, $N\geq2$ & ESTABLISHED & Nothing; dimension count \\
Prop.~7.7 & Simultaneous trivialization holds exactly for coboundary transports & ESTABLISHED & Nothing; proved both directions \\
Hyp.~7.8 & Flat comparison & HYPOTHESIS & A declared choice; excludes nontrivial holonomy. Used by the aggregation closure, which must restate it \\
Prop.~7.9 & Assembled precision blocks of a directed linear-Gaussian model & ESTABLISHED & Nothing; proved \\
Prop.~7.10 & The interaction form fails: asymmetry and sign witnesses & ESTABLISHED & Nothing; arithmetic \\
Prop.~7.11 & Exact global condition for family membership & ESTABLISHED & Nothing; proved \\
Eq.~\eqref{eq:gauss-global-cancellation-witness} & Global membership need not hold factor by factor & ESTABLISHED & Exact scalar counterexample with a non-idempotent gain \\
Prop.~7.12 & Edge-local membership with singular gains allowed holds exactly for $\Lambda_i$-orthogonal projections & ESTABLISHED & Nothing; proved. The accompanying sweep is corroboration only \\
Cor.~7.12a & Invertible-gain edge-local membership holds exactly at \(T_i=I\) & ESTABLISHED & An invertible idempotent is the identity \\
Hyp.~7.13 & The global interaction form is adopted as a restriction & HYPOTHESIS & Exact directed-model scope is Prop.~7.11; it does not imply factorwise agreement \\
Hyp.~7.13a & Optional factorwise-agreement realization & HYPOTHESIS & Strictly stronger than global membership; singular projections are allowed only in the relaxed gain class \\
Prop.~7.14 & The family is nonempty; explicit agreement-factor construction & ESTABLISHED & Nothing; proved \\
Prop.~7.14 & Measured contrast between the untrivialized and trivialized agreement weights & \status{NUMERICAL} & Computation only, seed $20260728$, the trivialized weight serving as control; the statement is proved by Prop.~7.14 \\
Sec.~7.5 & Conditioning of one sampling scheme & \status{NUMERICAL} & Computation only, with two controls; no conditioning bound is proved or claimed \\
Prop.~7.15 & Unrestricted matrix-weighted Kron closure fails & ESTABLISHED & Exact rational \(N=3,K=2\) witness with an SPD fine precision and nonsymmetric manufactured coupling \\
Prop.~7.16 & The common-orthogonal-eigenbasis subfamily is Kron closed & ESTABLISHED & Channelwise scalar loopy-Laplacian proof; self terms must share the eigenbasis \\
Open~7.17 & Classification of maximal Kron-closed subfamilies & OPEN & Characterize parameter sets preserving symmetry, sign, invertibility, and their own hypotheses under every elimination \\
\end{longtable}
}
