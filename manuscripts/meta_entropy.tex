...
of the functional whose minimisation over $\betaij$ returns the softmax \cref{eq:softmax}.
...
$\KL{\rho^{\star}}{\mu_1}=-H(\rho^{\star})+\log\mathrm{Vol}(\{\lVert\bmu\rVert\le R\})$, and only in the
...
the incoming kernel $K(\cdot,y)$; the directedness of the row coupling keeps these two terms
...
+\sqrt{2\Tmeta}\bigl(\sqrt{g^{-1}}\bigr)^{i}{}_{k}\dd W^{k}.
\end{equation}
