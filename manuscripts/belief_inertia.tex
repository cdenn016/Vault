...
Why do some beliefs resist change more than others? Why does influence itself seem to harden the minds and poison the empathies of those who wield it? Some beliefs are stiff while others readily sway. While confident beliefs clearly possess more "cognitive inertia" than uncertain ones, a principled mathematical foundation for this intuitive phenomenon remains elusive. Current theories of belief updating, from Bayesian inference \citep{jaynes2003probability} to predictive coding \citep{Friston2010,clark2013whatever}, model belief change as gradient descent. This is a purely dissipative process where beliefs flow toward lower free energy without momentum, inertia, or dynamics. Though enormously successful across neuroscience \citep{friston2016active}, psychology \citep{hohwy2013predictive}, and machine learning \citep{millidge2021predictive}, this framework does not account for inertial or oscillatory dynamics.
...
\begin{align}
\mathcal{F}[\{q_i\}, \{\phi_i\}] &= \sum_i \underbrace{\alpha_i D_{\mathrm{KL}}(q_i \| p_i)}_{\text{Prior beliefs}} + \sum_{i,j} \underbrace{\beta_{ij} D_{\mathrm{KL}}(q_i \| \Omega_{ij} \cdot q_j)}_{\text{Social alignment}} \\
&\quad - \sum_i \underbrace{\mathbb{E}_{q_i}[\log p(o_i \mid c_i)]}_{\text{Sensory evidence}}
\end{align}

where $c_i$ denotes the hidden causes (latent state) that agent $i$ infers, with $\mu_i = \mathbb{E}_{q_i}[c_i]$ being the posterior mean belief about these causes. The coefficient $\beta_{ij}$ represents the attention agent $i$ places in agent $j$'s beliefs and we take $p_i$ to be quasi-static. The scalar $\alpha_i$ is agent $i$'s self-coupling weight, the precision of its attachment to its own prior; it is set to the canonical default $\alpha_i = 1$ throughout the main-text derivations and promoted to a state-dependent precision in Section~\ref{sec:state_dependent_precision}. The attention naturally emerges as
...
\begin{equation}
\mathcal{F} = \underbrace{\sum_i \alpha_i D_{\mathrm{KL}}(q_i \| p_i)}_{\text{complexity}} - \underbrace{\sum_i \mathbb{E}_{q_i}[\log p(o_i|c_i)]}_{\text{accuracy}} + \underbrace{\sum_{i,k} \beta_{ik} D_{\mathrm{KL}}(q_i \| \Omega_{ik}\cdot q_k)}_{\text{consensus}}
\end{equation}
...
This four-part structure has transparent intuitive meaning. The term $\bar{\Lambda}_{p_i}$ represents \textbf{prior inertia}, the resistance arising from the cost of deviating from deep expectations; it carries the self-coupling weight $\alpha_i$, set to its default $1$ here and promoted to a state-dependent precision in Section~\ref{sec:state_dependent_precision}, where the prior block becomes $\alpha_i^* \bar{\Lambda}_{p_i}$. The term $\Lambda_{o_i}$ represents \textbf{sensory inertia}, grounding through observation whereby precise senses anchor beliefs. The term $\sum_k \beta_{ik}\tilde{\Lambda}_{q_k}$ represents \textbf{incoming social inertia}, the effect of being pulled toward confident neighbors. Finally, the term $\sum_j \beta_{ji}\Lambda_{q_i}$ represents \textbf{outgoing social inertia}, the recoil from exerting influence on others.
...
The consequence for the dynamics turns on a single fact: the envelope theorem cleans the gradient but not the Hessian. Differentiating the stationary objective, the regularizer contribution and the implicit $\partial \alpha_i^* / \partial \mu_i$ cancel, leaving the self-coupling force $\alpha_i^* \nabla_{\mu_i} D_{\mathrm{KL}}(q_i \| p_i)$. This envelope force is the gradient of the reduced self-energy $\alpha_i^* D_{\mathrm{KL}} + R(\alpha_i^*)$; the companion framework \citep{Dennis2025trans} instead differentiates the bare weighted divergence $\alpha_i^* D_{\mathrm{KL}}$ alone and reports the prefactor $(\alpha_i^*)^2 b_0 / c_0$, which equals $\partial_{D_{\mathrm{KL}}}(\alpha_i^* D_{\mathrm{KL}})$ and differs from $\alpha_i^*$ only in which objective is differentiated; the two agree on the dynamics. Recomputing $\alpha_i^*$ from the current belief at each inference step and treating it as fixed in the second variation, the prior block of the mass is rescaled,
\begin{equation}
...
\end{equation}
which is the natural generalization of the mass tensor and reduces to it at $\alpha_i^* = 1$. The weight rescales only the prior term, the second variation of $D_{\mathrm{KL}}(q_i \| p_i)$; the observation and social terms descend from separate divergences and are untouched. The same rescaling reaches the covariance sector through the prior's contribution to Eq.~\eqref{eq:mass_covariance}: its leading unit coefficient becomes $\alpha_i^*$, so $[\mathbf{M}^\Sigma]_{ii} = \frac{1}{2}(\Lambda_{q_i} \otimes \Lambda_{q_i})(\alpha_i^* + \sum_k \beta_{ik} + \sum_j \beta_{ji})$, the prior weight rescaling in both sectors alike.
...
\begin{align}
[\mathbf{M}^\mu]_{ii}' &= \alpha_i\bar{\Lambda}_{p_i}' + \sum_k \beta_{ik}\tilde{\Lambda}_{q_k}' + \sum_j \beta_{ji}\Lambda_{q_i}' \nonumber\\
&= \alpha_i g_i^{-T}\bar{\Lambda}_{p_i} g_i^{-1} + \sum_k \beta_{ik} g_i^{-T}\tilde{\Lambda}_{q_k} g_i^{-1} + \sum_j \beta_{ji} g_i^{-T}\Lambda_{q_i} g_i^{-1} \nonumber\\
&= g_i^{-T} \left[\alpha_i\bar{\Lambda}_{p_i} + \sum_k \beta_{ik}\tilde{\Lambda}_{q_k} + \sum_j \beta_{ji}\Lambda_{q_i}\right] g_i^{-1} \nonumber\\
...
\begin{align}
[\mathbf{M}^\Sigma]_{ii}' &= \frac{1}{2}(\Lambda_{q_i}' \otimes \Lambda_{q_i}') \cdot \left(\alpha_i + \sum_k \beta_{ik} + \sum_j \beta_{ji}\right) \nonumber\\
...
\begin{equation}
\boxed{
\mathcal{F}[\{q_i\}] = \sum_i \alpha_i D_{\mathrm{KL}}(q_i \| p_i) + \sum_{i,k} \beta_{ik} D_{\mathrm{KL}}(q_i \| \Omega_{ik}\cdot q_k) - \sum_i \mathbb{E}_{q_i}[\log p(o_i \mid \theta)]
}
\label{eq:full_free_energy}
\end{equation}
%
The three terms represent, respectively, \textbf{prior anchoring} (deviation from internal world-model), \textbf{social consensus} (alignment with other agents via gauge-covariant transport), and \textbf{sensory evidence} (grounding in observations). The scalar $\alpha_i$ is the self-coupling weight on the prior-anchoring term, set to $\alpha_i = 1$ in the main-text derivations and promoted to the state-dependent precision $\alpha_i^*$ of Section~\ref{sec:state_dependent_precision}; we carry it explicitly through the gradient and mass below so that the inertial mass specializes to that section by the single substitution $\bar{\Lambda}_{p_i} \to \alpha_i \bar{\Lambda}_{p_i}$.
...
\begin{equation}
\boxed{
[\mathbf{M}^\mu]_{ii} = \underbrace{\alpha_i\bar{\Lambda}_{p_i}}_{\text{prior}} + \underbrace{\sum_k \beta_{ik}\tilde{\Lambda}_{q_k}}_{\text{incoming social}} + \underbrace{\sum_j \beta_{ji}\Lambda_{q_i}}_{\text{outgoing recoil}} + \underbrace{\Lambda_{o_i}}_{\text{sensory}}
}
...
By Schwarz's theorem, this Hessian is symmetric: using $\Lambda = \Lambda^T$ one verifies that $[\mathbf{M}^\mu]_{ik} = ([\mathbf{M}^\mu]_{ki})^T$ holds for any $\beta_{ik}$ and any invertible $\Omega_{ik}$.\footnote{The reciprocal (recoil) term $\sum_j \beta_{ji}\Lambda_{q_i}$ is what makes the assembled mass symmetric and the force conservative even when the realized attention is asymmetric ($\beta_{ik} \neq \beta_{ki}$). An implementation that retains only the incoming term $\sum_k \beta_{ik}\tilde{\Lambda}_{q_k}$ restores a symmetric, single-Hamiltonian structure (in the metric-preserving or flat-gauge regime, and only up to an overall scale) after symmetrizing the attention, $\beta \to \tfrac{1}{2}(\beta + \beta^\top)$; it does not reproduce the full assembled mass exactly.}
...
From consensus (as receiver and sender), identical contributions arise. The prior contribution carries the self-coupling weight $\alpha_i$ (the term being $\alpha_i D_{\mathrm{KL}}(q_i \| p_i)$), so it enters the covariance-sector mass with coefficient $\alpha_i$ rather than unity; at the default $\alpha_i = 1$ this is the familiar prior contribution.
...
\begin{equation}
[\mathbf{M}^\mu]_{ii} = \underbrace{\alpha_i\bar{\Lambda}_{p_i}}_{\text{prior anchoring}} + \underbrace{\sum_k \beta_{ik}\Omega_{ik}^{-T}\Lambda_{q_k}\Omega_{ik}^{-1}}_{\text{incoming consensus}} + \underbrace{\sum_j \beta_{ji}\Lambda_{q_i}}_{\text{outgoing recoil}} + \underbrace{\Lambda_{o_i}}_{\text{sensory grounding}}
...
\begin{equation}
\boxed{
M_i = \alpha_i\bar{\Lambda}_{p_i} + \sum_k \beta_{ik}\tilde{\Lambda}_{q_k} + \sum_j \beta_{ji}\Lambda_{q_i} + \Lambda_{o_i}
}
\label{eq:effective_mass}
\end{equation}

The term $\alpha_i\bar{\Lambda}_{p_i}$ represents \textbf{bare mass}, providing inertia against deviation from the prior (with self-coupling weight $\alpha_i = 1$ by default; Section~\ref{sec:state_dependent_precision} promotes it to the state-dependent $\alpha_i^*$). The term $\sum_k \beta_{ik}\tilde{\Lambda}_{q_k}$ represents \textbf{incoming relational mass}, the inertia from being "pulled" by neighbors. The term $\sum_j \beta_{ji}\Lambda_{q_i}$ represents \textbf{outgoing relational mass}, the inertia from "pulling" neighbors (recoil). Finally, $\Lambda_{o_i}$ represents \textbf{sensory mass}, the inertia from grounding in observations.
