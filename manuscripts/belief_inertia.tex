...
\begin{equation}
\mathsf g = \mathrm{diag}(g_1, g_2, \ldots, g_N) \in G^N
\end{equation}
...
The locally matched covariance-stiffness block involves Kronecker products. Using the contragredient precision law $\Lambda_{q_i}'=g_i^{-T}\Lambda_{q_i}g_i^{-1}$ gives
\begin{align}
...
\begin{equation}
\boxed{(\mathbf H_F^\Sigma)' = (\mathsf g^{-T}\otimes\mathsf g^{-T})\mathbf H_F^\Sigma(\mathsf g^{-1}\otimes\mathsf g^{-1})}
\end{equation}
...
\begin{equation}
(\mathbf H_F^{\mu\Sigma})'=\mathsf g^{-T}\mathbf H_F^{\mu\Sigma}(\mathsf g^{-1}\otimes\mathsf g^{-1})
\end{equation}
...
Velocities therefore transform as vectors, $\dot{\mu}'=\mathsf g\dot{\mu}$, consistently with $\mu'=\mathsf g\mu$, for any $G\subseteq\mathrm{GL}(d)$. If a kinetic metric $\mathbf M$ is separately chosen with the congruence law $\mathbf M'=\mathsf g^{-T}\mathbf M\mathsf g^{-1}$, then $T=\frac12\dot\mu^T\mathbf M\dot\mu$ is gauge invariant.
...
\begin{equation}
\mathbf{M}'=\mathsf g^{-T}\mathbf{M}\mathsf g^{-1}
\end{equation}
(On $\mathrm{SO}(d)$, where $g^{-T}=g$, these reduce to the orthogonal congruences $g\pi$, $g\Pi g^T$, and $\mathsf g\mathbf M\mathsf g^T$.)
