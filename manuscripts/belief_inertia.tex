...
\begin{abstract}
We formulate an engineered gauge-covariant consensus energy for distribution-valued beliefs represented in heterogeneous local frames. Its relational weights follow from entropy-retaining attention optimization, whose rowwise elimination yields a reduced log-partition functional. Around local Gaussian consensus, the corresponding loss Hessian separates into prior, sensory, incoming relational, and outgoing relational or recoil stiffness. We take the unweighted product Fisher--Rao metric as the primary first-order geometry. That primary flow yields continuous-time DeGroot only for fixed symmetric coupling; a reversible nonuniform extension requires an additional stationary-measure-weighted product Fisher metric, equivalently a fixed-label joint family or agent-specific rates. Persistent anchors give a restricted Friedkin--Johnsen stationary equilibrium independently of the positive flow metric, although matching the standard transient generator retains the same weighted-metric or rate requirement. Finite-temperature attention gives a soft analog of bounded confidence. Belief inertia enters only through a separate kinetic-metric postulate: a positive local restriction of the stiffness may be adopted as kinetic geometry, but if the same functional supplies both kinetic mass $M$ and restoring Hessian $H_F$, then $M=H_F$ makes the generalized harmonic spectrum degenerate, $\omega^2=1$ up to scale. In the stated unanchored, symmetric reciprocal two-cluster reduction, separated clusters retain nonzero mutual influence and are metastable rather than stably polarized. This construction is a local mathematical model: it distinguishes intrinsic Fisher geometry, loss curvature, and optional kinetic structure, and it treats social and psychological interpretations as mechanism-specific hypotheses whose empirical status remains open.
\end{abstract}
...
E4 & Under the primary unweighted product Fisher metric, transient DeGroot recovery requires fixed symmetric coupling. A standard nonuniform reversible transient requires an additional $\rho$-weighted product Fisher metric (or an equivalent fixed-label joint family or agent-specific rate choice). The restricted anchored Friedkin--Johnsen equilibrium is a stationary point of the reversible scalar energy and is independent of the positive flow metric; the general directed iteration is not derived. \\
...
The metric in Eq.~\eqref{eq:primary_fisher_flow} is the unweighted product metric over the agent-indexed Gaussian factors.  In the fixed common-covariance mean sector its stacked block is $\mathbf G_{\mathrm{prod}}=\sigma^{-2}(I_N\otimes I_d)$.  A stationary-measure-weighted block $\mathbf G_\rho=\sigma^{-2}(D_\rho\otimes I_d)$ is an additional geometry: it is the Fisher metric of the fixed-label joint family $P(i,x)=\rho_iq_i(x)$, and it is not the unmodified primary product flow when $\rho$ is nonuniform.
...
For the primary first-order dynamics, linearization at a critical point gives $\dot\delta=-\eta\mathbf G^{-1}\mathbf H_F\delta$.  At fixed $\mathbf G$ and $\eta$, increasing a positive modal stiffness reduces the static displacement under a matched constant force and increases the linear relaxation rate.  Stiffness therefore does not by itself imply slower revision.  Slower dynamics require an independently specified mobility or learning rate, damping law, kinetic metric, or slow explanatory state.
...
Interpreting this mathematical coast as belief perseverance is a falsifiable mechanism-specific hypothesis, not a consequence of the primary Fisher flow \citep{anderson1980perseverance}.  A test would have to manipulate or estimate $m$ separately from $k$ and $\gamma$ and then compare the observed post-drive velocity decay with Eq.~\eqref{eq:conditional_coasting_time}.
...
The social reduction below supplies a separate falsification target.  With fixed symmetric weights, no anchors, and the primary unweighted product metric, the predicted decay rates are $\eta_\mu\lambda_s\ell_k$.  For nonuniform reversible weights, the same DeGroot generator and rate statement require the additional metric $\mathbf G_\rho$ (or agent-specific rates $\eta_i\propto\rho_i^{-1}$); the primary unweighted flow instead retains a $D_\rho$ factor.  In the stated unanchored, symmetric reciprocal two-cluster reduction, persistent separation under a connected, positive, finite-temperature kernel would contradict the purely attractive model.  Conversely, persistence accompanied by a measured hard support break, a persistent anchor, signed influence, or active sampling would test an extended mechanism rather than the theorem proved here.
...
The social reductions start from Fisher--Rao flow, not Hessian-preconditioned descent and not an overdamped limit of the optional kinetic theory.  The primary metric is the unweighted product metric in Eq.~\eqref{eq:primary_fisher_flow}; it gives the symmetric DeGroot case below.  The nonuniform reversible extension changes the geometry to a stationary-measure-weighted product Fisher metric, or equivalently changes the agent-specific rates.  Row-normalized attention specifies relative allocation among candidate sources, so we introduce an unnormalized social strength $\lambda_s>0$ outside the attention row.  This parameter changes the rate of social relaxation without being erased by row normalization.

\subsection{Symmetric and Reversible DeGroot Flow}

\begin{proposition}[Stationary-measure-weighted reversible DeGroot reduction]
...
\end{equation}
and use the additional $\rho$-weighted product Fisher metric
$\mathbf G_\rho=\sigma^{-2}(D_\rho\otimes I)$.  Equivalently, this is the mean block of the fixed-label joint family $P(i,x)=\rho_iq_i(x)$; the same cancellation can be implemented under the unweighted product metric by choosing agent-specific learning rates $\eta_i=\eta_\mu/\rho_i$.  The resulting natural-gradient flow is
\begin{equation}
\boxed{\dot{\boldsymbol\mu}
=-\eta_\mu\lambda_s[(I-W)\otimes I]\boldsymbol\mu.}
\label{eq:reversible_degroot_flow}
\end{equation}
For symmetric row-stochastic $W$, $\rho$ is uniform and $\mathbf G_\rho$ differs from the primary product metric only by an absorbable global scale.  Forward Euler with $h\eta_\mu\lambda_s=1$ after that scale is absorbed gives the discrete DeGroot step $\boldsymbol\mu^{n+1}=W\boldsymbol\mu^n$.
\end{proposition}
...
\end{equation}
The inverse $i$th block of the additional weighted metric is $\sigma^2\rho_i^{-1}I$.  Substitution into the natural-gradient law cancels both $\sigma^{-2}$ and $\rho_i$, yielding Eq.~\eqref{eq:reversible_degroot_flow}.  The Euler statement follows directly.
\end{proof}

The metric qualification is load bearing.  If the unweighted primary block $\mathbf G_{\mathrm{prod}}=\sigma^{-2}(I_N\otimes I_d)$ is retained with one common learning rate, the same energy gives
\begin{equation}
\dot{\boldsymbol\mu}
=-\eta_\mu\lambda_s[D_\rho(I-W)\otimes I]\boldsymbol\mu,
\label{eq:unweighted_reversible_flow}
\end{equation}
so $D_\rho$ does not cancel.  Equation~\eqref{eq:reversible_degroot_flow} is therefore not the unmodified primary flow for a general nonuniform reversible chain.
...
\begin{proposition}[Metric-independent anchored reversible equilibrium]
\label{prop:restricted_fj}
Retain the reversible matrix, flat-transport, fixed-common-covariance, and reciprocal-energy assumptions of Proposition~\ref{prop:reversible_degroot}, but do not fix a flow metric.  Let $u_i$ be a persistent prior mean and let $a_i\geq0$ be an agent-indexed anchor precision or coupling, with at least one anchor in every connected component.  Add
\begin{equation}
...
Because every positive-definite flow metric has the same zero set as the scalar-energy gradient, Eq.~\eqref{eq:restricted_fj_equilibrium} is independent of that metric.  The qualification reappears only for transient matching: $\mathbf G_\rho$ or $\eta_i\propto\rho_i^{-1}$ gives the standard reversible Friedkin--Johnsen generator, whereas the primary unweighted metric retains a left factor $D_\rho$.  The result is an equilibrium correspondence, not a derivation of the general directed Friedkin--Johnsen iteration.  A common anchor precision produces only a common susceptibility.  Heterogeneous $s_i$ requires agent-indexed prior precision $a_i$, agent-indexed social coupling, or another explicitly heterogeneous mechanism; row normalization alone cannot produce it.
...
\midrule
DeGroot & Derived subclass & Primary unweighted metric for fixed symmetric $W$; nonuniform reversible $W$ only with $\mathbf G_\rho$ or $\eta_i\propto\rho_i^{-1}$; general directed $W$ excluded. \\
Friedkin--Johnsen & Restricted equilibrium & Stationary point of the reversible anchored scalar energy, independent of the positive flow metric; matching the standard transient requires $\mathbf G_\rho$ or $\eta_i\propto\rho_i^{-1}$; heterogeneous susceptibility requires agent-indexed anchor precision or coupling. \\
...
\bottomrule
\end{tabular}
\caption{Proof status of the social-dynamics claims.  The symmetric DeGroot transient uses the primary unweighted product metric; the standard nonuniform reversible transient requires the additional $\rho$-weighted metric or its equivalent rate convention.  The anchored stationary point is metric-independent.  None depends on the kinetic postulate.}
\label{tab:rigor}
\end{table}
...
Classical opinion dynamics provides the first-order baseline: DeGroot averaging, Friedkin--Johnsen anchoring, and the hard-support Hegselmann--Krause and Deffuant bounded-confidence rules \citep{degroot1974,friedkin1990,hegselmann2002opinion,deffuant2000}. This paper does not recover those families in full. It proves DeGroot for fixed symmetric coupling under the primary unweighted metric and for nonuniform reversible coupling only under the additional $\rho$-weighted metric, derives the corresponding restricted anchored equilibrium, and treats finite-temperature Gibbs attention only as soft similarity selectivity. The comparative contribution is thus not a universal replacement for classical models, but one gauge-transported Gaussian KL consensus framework that links a precise first-order subclass to optimized attention, four-part local stiffness, and a separately conditional kinetic extension.

\section{Discussion}

The theorem-level result is a gauge-covariant Gaussian consensus objective with entropy-retaining optimized attention and a local four-part loss stiffness. Its primary dynamics use the unweighted product Fisher--Rao metric. In the flat, fixed-covariance case, that primary flow yields continuous-time DeGroot for symmetric coupling; a standard nonuniform reversible DeGroot transient requires $\mathbf G_\rho$ or equivalent agent-specific rates. Persistent anchors yield a restricted Friedkin--Johnsen stationary equilibrium independently of the positive flow metric, while the same weighted-metric or rate choice is needed only to match its standard transient generator. Finite-temperature Gibbs attention is a soft bounded-confidence analog, and its positive tail gives metastable rather than stable separation only in the stated unanchored, symmetric reciprocal two-cluster reduction.
...
\textbf{Reciprocity and social reductions.} The derived social limits require flat transport and common fixed covariance. The primary unweighted product metric yields the standard DeGroot transient only for symmetric coupling; matching the nonuniform reversible transient additionally requires $\mathbf G_\rho$ or equivalent agent-specific rates. The restricted anchored stationary equilibrium is metric-independent, but general directed DeGroot or Friedkin--Johnsen dynamics, hard bounded confidence, persistent polarization, signed influence, diffusion, and active evidence selection require additional structure.
...
This paper establishes a gauge-covariant Gaussian KL consensus energy, the unique Gibbs attention minimizer of its entropy-retaining row objective, and a local four-part mean stiffness comprising prior, sensory, incoming relational, and outgoing relational contributions. The primary dynamics use the unweighted product Fisher--Rao metric. Under flat common covariance they yield continuous-time DeGroot for fixed symmetric coupling; matching a standard nonuniform reversible transient requires the additional $\rho$-weighted product metric. The anchored Friedkin--Johnsen stationary equilibrium is independent of the positive flow metric, although its standard transient retains that metric or rate qualification. Finite-temperature attention supplies soft similarity selectivity and, in the stated two-cluster reduction, metastable rather than stable separation.
