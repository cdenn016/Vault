...
For each receiver $i$, let $\pi_i$ be a strictly positive probability vector on its finite candidate-source set, let $\beta_i$ lie in the same probability simplex, and fix $\tau>0$. A hard mask is represented by removing an index from that set rather than assigning a positive-weight formula at $\pi_{ij}=0$. The authoritative scalar objective is
\begin{equation}
\boxed{
...
When $\pi_i$ and $\tau$ are fixed, direct differentiation of the log-partition function proves the envelope identity
\begin{equation}
\boxed{
...
\end{equation}
For fixed $\pi_i$ and $\tau$, Eq.~\eqref{eq:attention_weights} gives
\begin{equation}
d\beta_{ij}^*
=-\tau^{-1}\beta_{ij}^*
...
The complete scalar is the entropy-retaining objective $\mathcal F_{\mathrm{full}}$ in Eq.~\eqref{eq:canonical_full_vfe}, or its adaptive-prior extension obtained by the replacement in Eq.~\eqref{eq:adaptive_self_coupling}. In the frozen-attention calculation below, the categorical entropy term remains part of the scalar but is constant with respect to the Gaussian belief coordinates. Denoting the full state vector $\xi = (\mu_1, \ldots, \mu_N, \Sigma_1, \ldots, \Sigma_N)$, this section considers the coordinate Hessian

\begin{equation}
\mathbf{M} = \frac{\partial^2 \mathcal{F}_{\mathrm{full}}}{\partial\xi\partial\xi^\top}
\end{equation}
...
This display is the frozen-attention coordinate Hessian, not the Hessian of the envelope-reduced row. When attention is optimized, the canonical first variation is Eq.~\eqref{eq:reduced_vfe_envelope}; attention response re-enters only at second order through the reduced log-partition curvature. The local-stiffness result below therefore states explicitly whether attention is frozen or envelope-eliminated rather than differentiating the entropy-suppressed product as though it were the canonical objective.
...
\end{equation}
Both distributions are normalized when $q_i$, every $r_{ij}$, $\beta_i$, and $\pi_i$ are normalized. Direct expansion gives the exact unit-temperature identity
\begin{equation}
\KL(Q_i\|P_i)
