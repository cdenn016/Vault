...
E4 & Under the primary unweighted product Fisher metric, DeGroot recovery requires fixed symmetric coupling. Reversible nonuniform coupling requires an additional $\rho$-weighted product Fisher metric (or an equivalent fixed-label joint family or agent-specific rate choice); Friedkin--Johnsen is only the corresponding restricted anchored equilibrium. \\
...
Retain the assumptions of Proposition~\ref{prop:reversible_degroot}, including its additional $\rho$-weighted metric (or equivalent agent-specific rates).  Let $u_i$ be a persistent prior mean and let $a_i\geq0$ be an agent-indexed anchor precision or coupling, with at least one anchor in every connected component.  Add
\begin{equation}
...
Friedkin--Johnsen & Restricted equilibrium & Anchored system inheriting the same symmetric/weighted-reversible metric scope; heterogeneous susceptibility requires agent-indexed anchor precision or coupling. \\
...
\bottomrule
\end{tabular}
\caption{Proof status of the social-dynamics claims.  The symmetric DeGroot row uses the primary unweighted product metric; the nonuniform reversible rows require the additional $\rho$-weighted metric or its equivalent rate convention.  None depends on the kinetic postulate.}
\label{tab:rigor}
\end{table}
...
The theorem-level result is a gauge-covariant Gaussian consensus objective with entropy-retaining optimized attention and a local four-part loss stiffness. Its primary dynamics use the unweighted product Fisher--Rao metric. In the flat, fixed-covariance case, that primary flow yields continuous-time DeGroot for symmetric coupling; a nonuniform reversible chain yields DeGroot only after adopting $\mathbf G_\rho$ or equivalent agent-specific rates. Persistent anchors yield the correspondingly restricted Friedkin--Johnsen equilibrium. Finite-temperature Gibbs attention is a soft bounded-confidence analog, and its positive tail gives metastable rather than stable separation only in the stated unanchored, symmetric reciprocal two-cluster reduction.
...
\textbf{Reciprocity and social reductions.} The derived social limits require flat transport and common fixed covariance. The primary unweighted product metric yields DeGroot only for symmetric coupling; the nonuniform reversible result additionally requires $\mathbf G_\rho$ or equivalent agent-specific rates. General directed DeGroot or Friedkin--Johnsen dynamics, hard bounded confidence, persistent polarization, signed influence, diffusion, and active evidence selection require additional structure.
...
This paper establishes a gauge-covariant Gaussian KL consensus energy, the unique Gibbs attention minimizer of its entropy-retaining row objective, and a local four-part mean stiffness comprising prior, sensory, incoming relational, and outgoing relational contributions. The primary dynamics use the unweighted product Fisher--Rao metric. Under flat common covariance they yield continuous-time DeGroot for fixed symmetric coupling; nonuniform reversible DeGroot requires the additional $\rho$-weighted product metric, and the anchored Friedkin--Johnsen equilibrium inherits that scope. Finite-temperature attention supplies soft similarity selectivity and, in the stated two-cluster reduction, metastable rather than stable separation.
