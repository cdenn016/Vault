...
\end{equation}
with absolute continuity taken relative to the mixed reference in \cref{eq:vfe4-latent-reference-measure}. Equivalently, $Q_\psi^{(r)}$ assigns zero mass to every set on which the posterior density vanishes. In particular, a recognition source probability must vanish wherever the corresponding $\pi_t^z$ or $\pi_t^m$ vanishes. Assume also
\begin{equation}
...
\end{equation}
On the interior of the regular Gaussian family, the log normalizer $A(\eta)$ and its Legendre dual $A^*(\tau)$ satisfy
\begin{equation}
\tau=\nabla_\eta A(\eta),
\qquad
\eta=\nabla_\tau A^*(\tau).
