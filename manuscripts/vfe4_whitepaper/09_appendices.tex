...
\end{equation}
No equality $d_z=d_m$ is used.  This check is the source-level oracle for later probability kernels: a kernel over $E_{z,t}$ must receive a location in $E_{z,t}$ and a covariance or precision endomorphism of $E_{z,t}$, while a kernel over $E_{m,t}$ must receive the corresponding model-fiber objects.
