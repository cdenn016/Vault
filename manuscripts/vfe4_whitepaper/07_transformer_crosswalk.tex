...
The categorical variables $a_t$ and $b_t$ make source selection part of the probability model. Their generative priors in the present joint are fixed with respect to realized latent content, so they define normalized mixture ancestry rather than content-dependent transformer attention. A causal content-dependent prior is a future normalized extension and not a current identity. Write $Y=(\boldsymbol z,\boldsymbol m,a,b)$, and let $Y_{\setminus a_t}$ and $Y_{\setminus b_t}$ denote the complete latent configuration with the named coordinate removed. The positive-prior source supports are
\begin{equation}
S_t^z
