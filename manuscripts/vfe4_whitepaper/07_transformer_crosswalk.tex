...
The categorical variables $a_t$ and $b_t$ make source selection part of the probability model. Write $Y=(\boldsymbol z,\boldsymbol m,a,b)$, and let $Y_{\setminus a_t}$ and $Y_{\setminus b_t}$ denote the complete latent configuration with the named coordinate removed. If an unrestricted recognition block may condition pointwise on the entire complement, its ELBO-coordinate optimum is the corresponding posterior full conditional. The two source coordinates are therefore exactly
\begin{equation}
\begin{aligned}
