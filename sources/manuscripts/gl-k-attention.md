...
- **BERT validation (App. E).** Frozen `BERT-base-uncased` across **105 passages** and five architectures: at the theory-predicted $\tau=19.0\approx 2\sqrt{d}$ ($d=64$), the flat-bundle KL attention $\beta^{\text{flat}}_{ij}=\operatorname{softmax}(-\|Q_i-K_j\|^2/\tau)$ agrees with dot-product attention at grand mean $\bar r = 0.804$ (95% CI $[0.771,0.838]$), entropy matching $H(\beta)/H(\alpha)=1.076$, key-norm bias Cohen's $d=1.43$, per-head temperature dispersion (RoBERTa median $\tau=25$). Treated as a consistency check, since high $r$ partly follows from an algebraic identity.
...
- **Symmetry-breaking simulations (App. G).** Six $\mathrm{SO}(3)$ ($\ell=4$) agents on a 2D base: without observations beliefs collapse to a rotationally invariant vacuum ($\mathrm{Var}(\|\mu\|)\to$ machine precision); with observations symmetry breaks and norms diverge. Also gives the full two-channel **model-channel free energy** $\mathcal{F}_{\text{full}}$ with belief/model alignment terms $\beta_{ij}, \gamma_{ij}$ (set $\gamma_{ij}=0$ here).
